...
export function decodeLz4Frame(bytes, options = {}) {
    ...
    if (options.checkChecksum !== false) {
        ...
        decodeAssert(storedHeaderChecksum === expected, "LZ4 header checksum mismatch", headerChecksumPosition);
    }
    ...
    for (;;) {
        ...
        const raw = (blockInfo & 2147483648) !== 0;
        const length = blockInfo & 2147483647;
        ...
        if (blockChecksum) {
            ...
            if (options.checkChecksum !== false) {
                decodeAssert(stored === xxhash32(block, 0), "LZ4 block checksum mismatch", reader.position - 4);
            }
        }
        ...
        decodeAssert(output.length - blockOutputStart <= maxBlockSize, "LZ4 data block output exceeds BD maximum", infoOffset);
    }
    const result = output.finish();
    if (expectedContentSize !== undefined) {
        decodeAssert(expectedContentSize === BigInt(result.length), "LZ4 content size mismatch", descriptorStart);
    }
    if (contentChecksum) {
        const stored = reader.readU32LE();
        if (options.checkChecksum !== false) {
            decodeAssert(stored === xxhash32(result, 0), "LZ4 content checksum mismatch", reader.position - 4);
        }
    }
    reader.assertEnd("trailing LZ4 frame bytes");
    return result;
}
export function encodeLz4FrameRaw(bytes) {
    const { descriptor, maxBlockSize } = chooseBlockDescriptor(bytes.length);
    const flags = 0x60;
    const header = Uint8Array.of(flags, descriptor);
    const writer = new ByteWriter(bytes.length + 16 + Math.ceil(bytes.length / maxBlockSize) * 4);
    ...
    for (let offset = 0; offset < bytes.length; offset += maxBlockSize) {
        ...
        if (compressed.length < block.length) {
            ...
            writer.writeU32LE((2147483648 + block.length) >>> 0);
...
function encodeLz4Block(input) {
    ...
    const hashLog = Math.min(HASH_LOG, Math.max(MIN_HASH_LOG, Math.ceil(Math.log2(input.length))));
    ...
    while (position <= lastMatchStart) {
        ...
        writeSequence(writer, input.subarray(anchor, position), position - candidate, matchEnd - position);
...
function encodeLastLiterals(input) {
    const writer = new ByteWriter(input.length + 2);
    writeLastLiterals(writer, input);
    return writer.toUint8Array();
}
function writeSequence(writer, literals, offset, matchLength) {
...
function writeLastLiterals(writer, literals) {
    writer.writeU8(Math.min(literals.length, 15) << 4);
    writeExtendedLength(writer, literals.length);
    writer.writeBytes(literals);
}
function writeExtendedLength(writer, length) {
    if (length < 15) {
        return;
    }
...
function hashSequence(input, offset, hashShift) {
    const sequence = (input[offset] |
        (input[offset + 1] << 8) |
        (input[offset + 2] << 16) |
        (input[offset + 3] << 24)) >>>
        0;
...
function equalFourBytes(input, left, right) {
    return (input[left] === input[right] &&
        input[left + 1] === input[right + 1] &&
        input[left + 2] === input[right + 2] &&
        input[left + 3] === input[right + 3]);
}
function decodeLz4BlockInto(input, output, prefixStart, outputLimit) {
    const reader = new ByteReader(input);
    while (reader.remaining > 0) {
        const token = reader.readU8();
        const literalLength = readExtendedLength(reader, token >>> 4);
        decodeAssert(literalLength <= outputLimit - output.length, "LZ4 data block output exceeds BD maximum", reader.position);
        ...
        decodeAssert(offset <= output.length - prefixStart, "LZ4 match offset is out of bounds", offsetPosition);
        const matchLength = 4 + readExtendedLength(reader, token & 0x0f);
        decodeAssert(matchLength <= outputLimit - output.length, "LZ4 data block output exceeds BD maximum", reader.position);
...
function readExtendedLength(reader, base) {
    let length = base;
    if (base !== 15) {
        return length;
    }
    for (;;) {
        ...
        length += extra;
        decodeAssert(Number.isSafeInteger(length), "LZ4 sequence length overflow", reader.position - 1);
        if (extra !== 255) {
            return length;
        }
    }
}
function decodeBlockMaximum(descriptor, offset) {
...
function chooseBlockDescriptor(length) {
