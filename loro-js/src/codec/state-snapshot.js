...
import { containerTypeFromRawByte, containerTypeToRawByte, decodeContainerId, encodeContainerId, readPostcardOptionalContainerId, writePostcardOptionalContainerId, } from "./container-id";
...
import { decodeBoolRle, decodeColumnarVecMaybeWrapped, decodeDeltaRleI32, decodeDeltaRleU32, decodeDeltaRleUsize, encodeBoolRle, encodeColumnarVec, encodeColumnarVecWrapped, encodeDeltaRleI32, encodeDeltaRleU32, encodeDeltaRleUsize, takeColumnarVec, } from "./serde-columnar";
import { decodeSstable, encodeSstable, } from "./sstable";
import { ContainerType, } from "./types";
import { readPostcardValue, readPostcardValueMap, readPostcardValues, writePostcardValue, writePostcardValueMap, writePostcardValues, } from "./value";
import { decodePostcardFrontiers, encodePostcardFrontiers } from "./version";
const MAX_STATE_ITEMS = 10000000;
const EMPTY_STATE_SENTINEL = Uint8Array.of(0x45);
const FRONTIERS_KEY = Uint8Array.of(0x66, 0x72);
const textEncoder = new TextEncoder();
export function decodeMapStateSnapshot(bytes) {
...
export function encodeMapStateSnapshot(state) {
    ...
    for (const metadata of state.metadata) {
        encodeAssert(!metadataByKey.has(metadata.key), `duplicate map metadata key ${metadata.key}`);
...
export function decodeListStateSnapshot(bytes) {
    const reader = new PostcardReader(bytes);
    ...
    decodeAssert(peerIndices.length === values.length &&
        counters.length === values.length &&
        lamportSubs.length === values.length, "list state ID length mismatch");
    const peerCount = BigInt(peers.length);
...
export function encodeListStateSnapshot(state) {
    ...
    const ids = state.ids;
    const peerIndices = new Array(ids.length);
    const counters = new Array(ids.length);
    const lamportSubs = new Array(ids.length);
    ...
    writePeerTable(output, state.peers);
    output.writeBytes(encodeColumnarVecWrapped([
...
export function decodeTextStateSnapshot(bytes) {
    const reader = new PostcardReader(bytes);
    ...
    decodeAssert(peerIndices.length === lengths.length &&
        counters.length === lengths.length &&
        lamportSubs.length === lengths.length, "text state span column length mismatch");
    const peerCount = BigInt(peers.length);
...
export function encodeTextStateSnapshot(state) {
    validateEncodedText(state);
    const spans = state.spans;
    const peerIndices = new Array(spans.length);
    const counters = new Array(spans.length);
    const lamportSubs = new Array(spans.length);
    const lengths = new Array(spans.length);
    ...
    output.writeBytes(encodeColumnarVec([
        encodeDeltaRleUsize(peerIndices),
        encodeDeltaRleI32(counters),
        encodeDeltaRleI32(lamportSubs),
...
export function decodeTreeStateSnapshot(bytes) {
    ...
    decodeAssert(nodePeerIndices.length === nodeCounters.length, "tree state node ID column length mismatch");
    ...
    decodeAssert(parentIndices.length === length &&
        lastSetPeerIndices.length === length &&
        lastSetCounters.length === length &&
        lastSetLamportSubs.length === length &&
        fractionalIndexIndices.length === length, "tree state node column length mismatch");
    ...
    const nodes = nodePeerIndices.map((peerIndex, index) => {
        ...
        decodeAssert(parentIndexPlusTwo <= parentIndexBound, "tree parent index out of range");
        decodeAssert(fractionalIndexIndex < positions.length, "tree fractional index out of range");
        return {
            peerIndex,
...
export function encodeTreeStateSnapshot(state) {
    validateEncodedTree(state);
    const nodes = state.nodes;
    const nodePeerIndices = new Array(nodes.length);
    const nodeCounters = new Array(nodes.length);
    const parentIndices = new Array(nodes.length);
    const lastSetPeerIndices = new Array(nodes.length);
    const lastSetCounters = new Array(nodes.length);
    const lastSetLamportSubs = new Array(nodes.length);
    const fractionalIndexIndices = new Array(nodes.length);
...
export function decodeMovableListStateSnapshot(bytes) {
    const reader = new PostcardReader(bytes);
    const values = readPostcardValues(reader);
    const peers = readPeerTable(reader.input);
    decodeAssert(reader.readUsize() === 4, "movable-list state must have four encoded fields");
    ...
    decodeAssert(invisibleListItems.length === positionIdEqualsElementId.length &&
        invisibleListItems.length === elementIdEqualsLastSetId.length, "movable-list state item column length mismatch");
    ...
    decodeAssert(listPeerIndices.length === listCounters.length &&
        listPeerIndices.length === listLamportSubs.length, "movable-list list ID column length mismatch");
    ...
    decodeAssert(elementPeerIndices.length === elementLamports.length, "movable-list element ID column length mismatch");
    ...
    decodeAssert(lastSetPeerIndices.length === lastSetLamports.length, "movable-list last-set ID column length mismatch");
    reader.assertEnd();
    ...
    validateDecodedMovableList(values.length, BigInt(peers.length), items, listItemIds, elementIds, lastSetIds);
    return {
...
export function encodeMovableListStateSnapshot(state) {
    validateEncodedMovableList(state);
    const items = state.items;
    const invisibleListItems = new Array(items.length);
    const positionIdEqualsElementId = new Array(items.length);
    const elementIdEqualsLastSetId = new Array(items.length);
    ...
    const listPeerIndices = new Array(listItemIds.length);
    const listCounters = new Array(listItemIds.length);
    const listLamportSubs = new Array(listItemIds.length);
    ...
    const elementPeerIndices = new Array(elementIds.length);
    const elementLamports = new Array(elementIds.length);
    ...
    const lastSetPeerIndices = new Array(lastSetIds.length);
    const lastSetLamports = new Array(lastSetIds.length);
...
export function decodeCounterStateSnapshot(bytes) {
...
export function encodeCounterStateSnapshot(state) {
    const output = new ByteWriter(8);
    output.writeU64LE(state.bits);
    return output.toUint8Array();
}
export function decodeContainerStateSnapshot(containerType, bytes) {
...
export function encodeContainerStateSnapshot(state) {
...
export function decodeContainerStateWrapper(bytes) {
    const input = new ByteReader(bytes);
...
export function encodeContainerStateWrapper(wrapper) {
    encodeAssert(sameContainerType(wrapper.containerType, wrapper.state.kind), "container wrapper type does not match its state");
    const output = new ByteWriter();
...
export function decodeStateSnapshotStore(bytes, options) {
    if (bytes.length === 0) {
        return { kind: "absent" };
    }
    ...
    for (const entry of decodeSstable(bytes, options)) {
        ...
        decodeAssert(sameContainerType(id.containerType, wrapper.containerType), "state container key and wrapper types differ");
        containers.push({ id, wrapper });
    }
    return { kind: "sstable", frontiers, containers };
}
export function encodeStateSnapshotStore(store, options) {
    ...
    const entries = store.containers.map(({ id, wrapper }) => {
        encodeAssert(sameContainerType(id.containerType, wrapper.containerType), "state container key and wrapper types differ");
...
function readPeerTable(reader) {
    const length = readUlebNumber(reader, MAX_STATE_ITEMS);
    decodeAssert(length <= Math.floor(reader.remaining / 8), "state peer table is truncated");
    const peers = [];
...
function writePeerTable(writer, peers) {
...
function readColumnarVecFrom(reader) {
    const remaining = reader.bytes.subarray(reader.position, reader.position + reader.remaining);
    const [columns, rest] = takeColumnarVec(remaining);
    reader.skip(remaining.length - rest.length);
    return columns;
}
function readStringVector(reader, label) {
...
function decodePostcardUsizeVector(bytes) {
    const reader = new PostcardReader(bytes);
    const length = reader.readUsize();
...
function encodePostcardUsizeVector(values) {
...
function collectSortedMapKeysForDecode(values, deletedKeys) {
...
function collectSortedMapKeysForEncode(values, deletedKeys) {
    const keys = mergeMapKeys(values, deletedKeys);
    for (let index = 1; index < keys.length; index += 1) {
...
function mergeMapKeys(values, deletedKeys) {
    const keys = new Array(values.length + deletedKeys.length);
    let offset = 0;
 ...
 */
function sortByUtf8Key(items, keyOf) {
...
function assertDecodedPeerIndex(index, peerCount, label) {
    decodeAssert(index < peerCount, `${label} peer index out of range`);
}
function assertEncodedPeerIndex(index, peerCount, label) {
    encodeAssert(index >= 0n && index < peerCount, `${label} peer index out of range`);
}
function validateDecodedText(text, spans, marks) {
    let markCount = 0;
    let textLength = 0;
    ...
    decodeAssert(unicodeScalarLength(text) === textLength, "text state Unicode length mismatch");
}
function validateEncodedText(state) {
    let markCount = 0;
    let textLength = 0;
    const peerCount = BigInt(state.peers.length);
    ...
    encodeAssert(unicodeScalarLength(state.text) === textLength, "text state Unicode length mismatch");
    for (const mark of state.marks) {
        encodeAssert(mark.keyIndex >= 0 && mark.keyIndex < state.keys.length, "text mark key index out of range");
    }
}
...
function validateEncodedTree(state) {
    const peerCount = BigInt(state.peers.length);
    ...
    for (const node of state.nodes) {
        ...
        encodeAssert(node.parentIndexPlusTwo >= 0n &&
            node.parentIndexPlusTwo <= parentIndexBound, "tree parent index out of range");
        encodeAssert(Number.isSafeInteger(node.fractionalIndexIndex) &&
            node.fractionalIndexIndex >= 0 &&
            node.fractionalIndexIndex < state.positions.length, "tree fractional index out of range");
    }
}
function validateDecodedMovableList(valueCount, peerCount, items, listItemIds, elementIds, lastSetIds) {
    ...
    decodeAssert(expectedListIds === listItemIds.length, "movable-list list ID count mismatch");
    ...
    decodeAssert(elementIds.length === expectedElementIds, "movable-list element ID count mismatch");
    decodeAssert(lastSetIds.length === expectedLastSetIds, "movable-list last-set ID count mismatch");
...
function validateEncodedMovableList(state) {
    const visibleCount = state.items.length === 0 ? 0 : state.items.length - 1;
    encodeAssert(state.values.length === visibleCount, "movable-list visible value count mismatch");
    ...
    for (const item of state.items) {
        encodeAssert(item.invisibleListItems >= 0n, "movable-list invisible count is negative");
        expectedListIds += Number(item.invisibleListItems);
    }
    encodeAssert(expectedListIds === state.listItemIds.length, "movable-list list ID count mismatch");
    let expectedElementIds = 0;
    let expectedLastSetIds = 0;
    ...
    encodeAssert(state.elementIds.length === expectedElementIds, "movable-list element ID count mismatch");
    encodeAssert(state.lastSetIds.length === expectedLastSetIds, "movable-list last-set ID count mismatch");
    const peerCount = BigInt(state.peers.length);
