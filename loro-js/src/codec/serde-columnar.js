...
export function encodeColumnarVec(columns) {
...
export function encodeColumnarVecWrapped(columns) {
    const writer = new ByteWriter();
    writeUleb128(writer, 1);
    writer.writeBytes(encodeColumnarVec(columns));
    return writer.toUint8Array();
}
export function decodeBoolRle(bytes) {
    const reader = new ByteReader(bytes);
    const values = [];
    let state = false;
...
export function encodeBoolRle(values) {
    const writer = new ByteWriter();
    if (values.length === 0) {
        return writer.toUint8Array();
    }
...
export function takeBoolRle(bytes, count) {
    assertCount(count);
    const reader = new ByteReader(bytes);
    const values = [];
    let state = false;
...
export function decodeRleU8(bytes) {
    return decodeAnyRle(bytes, readPostcardU8);
}
export function encodeRleU8(values) {
    return encodeAnyRleLiteral(values, writePostcardU8);
}
export function decodeRleU32(bytes) {
    return decodeAnyRle(bytes, readPostcardU32);
}
export function encodeRleU32(values) {
    return encodeAnyRleLiteral(values, writePostcardU32);
}
export function decodeAnyRleU32(bytes) {
    return decodeAnyRle(bytes, readPostcardU32);
}
export function encodeAnyRleU32(values) {
    return encodeAnyRleLiteral(values, writePostcardU32);
}
export function takeAnyRleU32(bytes, count) {
    return takeAnyRle(bytes, count, readPostcardU32);
}
export function decodeAnyRleU64(bytes) {
    return decodeAnyRle(bytes, readPostcardU64);
}
export function encodeAnyRleU64(values) {
    return encodeAnyRleLiteral(values, writePostcardU64);
}
export function decodeAnyRleUsize(bytes) {
    return decodeAnyRleU64(bytes);
}
export function encodeAnyRleUsize(values) {
    return encodeAnyRleU64(values);
}
export function takeAnyRleUsize(bytes, count) {
    return takeAnyRle(bytes, count, readPostcardU64);
}
export function decodeAnyRleI32(bytes) {
    return decodeAnyRle(bytes, readPostcardI32);
}
export function encodeAnyRleI32(values) {
    return encodeAnyRleLiteral(values, writePostcardI32);
}
export function takeAnyRleI32(bytes, count) {
    return takeAnyRle(bytes, count, readPostcardI32);
}
export function decodeDeltaRleU32(bytes) {
    return decodeDeltaNumber(bytes, 0, U32_MAX);
}
export function encodeDeltaRleU32(values) {
    return encodeDeltaNumber(values, assertU32);
}
export function decodeDeltaRleI32(bytes) {
    return decodeDeltaNumber(bytes, I32_MIN, I32_MAX);
}
export function encodeDeltaRleI32(values) {
    return encodeDeltaNumber(values, assertI32);
}
export function decodeDeltaRleUsize(bytes) {
    return decodeDelta(bytes, 0n, U64_MAX, identity);
}
export function encodeDeltaRleUsize(values) {
    return encodeDelta(values, assertUsize);
}
export function decodeDeltaRleIsize(bytes) {
    return decodeDelta(bytes, I64_MIN, I64_MAX, identity);
}
export function encodeDeltaRleIsize(values) {
    return encodeDelta(values, assertIsize);
}
export function decodeDeltaOfDeltaI64(bytes) {
    const reader = new ByteReader(bytes);
    ...
    if (first === undefined) {
        decodeAssert(lastUsedBits === 0 && bitstream.length === 0, "invalid empty delta-of-delta encoding");
        return [];
    }
...
export function takeDeltaOfDeltaI64(bytes, count) {
    assertCount(count);
    const reader = new ByteReader(bytes);
    const first = readPostcardOptionalI64(reader);
    decodeAssert(reader.remaining >= 1, "invalid delta-of-delta bytes", reader.position);
    ...
    if (count === 1) {
        ...
        decodeAssert(lastUsedBits === expectedLastUsed, "delta-of-delta last-used-bits mismatch");
    }
    return [values, bytes.subarray(bitstreamOffset + Math.ceil(bits.position / 8))];
}
export function encodeDeltaOfDeltaI64(values) {
    const writer = new ByteWriter();
    if (values.length === 0) {
...
function readColumnarVec(reader) {
    const count = readUlebNumber(reader, MAX_COLUMN_VALUES);
    const columns = [];
    for (let index = 0; index < count; index += 1) {
        columns.push(reader.readBytes(readUlebNumber(reader, 2147483647)));
    }
    return columns;
}
function decodeAnyRle(bytes, readValue) {
    const reader = new ByteReader(bytes);
    const values = [];
    while (reader.remaining > 0) {
...
function takeAnyRle(bytes, count, readValue) {
    assertCount(count);
    const reader = new ByteReader(bytes);
    const values = [];
    while (values.length < count) {
...
function readAnyRleSegment(reader, values, limit, readValue) {
...
function encodeAnyRleLiteral(values, writeValue) {
    const writer = new ByteWriter();
    if (values.length === 0) {
        return writer.toUint8Array();
    }
...
function encodeDelta(values, toBigInt) {
    const writer = new ByteWriter();
    if (values.length === 0) {
        return writer.toUint8Array();
    }
...
function decodeDelta(bytes, min, max, fromBigInt) {
    const reader = new ByteReader(bytes);
    const values = [];
    let value = 0n;
        ...
        if (useNumber) {
            if (typeof delta === "number") {
                ...
                if (Number.isSafeInteger(next)) {
                    valueNumber = next;
                    decodeAssert(next >= minNumber && next <= maxNumber, "delta RLE value is out of range");
    ...
    while (reader.remaining > 0) {
        ...
        decodeAssert(lengthBigInt <= BigInt(MAX_COLUMN_VALUES), "AnyRle segment is too long");
        const length = Number(lengthBigInt);
        decodeAssert(values.length + length <= MAX_COLUMN_VALUES, "AnyRle has too many elements");
        if (signedLength > 0n) {
...
function encodeDeltaNumber(values, validate) {
    const writer = new ByteWriter();
    if (values.length === 0) {
        return writer.toUint8Array();
    }
...
function decodeDeltaNumber(bytes, min, max) {
    const reader = new ByteReader(bytes);
    const values = [];
    let value = 0;
    const append = (delta) => {
...
function readPostcardU8(reader) {
    return reader.readU8();
}
function writePostcardU8(writer, value) {
    writer.writeU8(value);
}
function readPostcardU32(reader) {
    return readUlebNumber(reader, 4294967295);
}
function writePostcardU32(writer, value) {
...
function readPostcardU64(reader) {
    return readUleb128(reader, U64_MAX);
}
function writePostcardU64(writer, value) {
    writeUleb128(writer, value);
}
function readPostcardI32(reader) {
    const encoded = readUlebNumber(reader, 4294967295);
...
function writePostcardI32(writer, value) {
...
function readPostcardI64(reader) {
...
function writePostcardI128Number(writer, value) {
    if (Number.isSafeInteger(value) && value > -2147483648 && value < 2147483648) {
...
function readPostcardI128NumberOrBigInt(reader) {
    const start = reader.position;
    let encoded = 0;
    let multiplier = 1;
...
function writeVarintU128(writer, input) {
...
class BitReader {
    ...
    #bitBuffer = 0;
    #bitCount = 0;
    constructor(bytes, totalBits) {
        this.#bytes = bytes;
        this.#totalBits = totalBits;
    }
    static forCompleteStream(bytes, lastUsedBits) {
    ...
    readBitsNumber(count) {
        decodeAssert(Number.isSafeInteger(count) && count >= 0 && count <= 24, "invalid bit width");
    ...
    readBitsBigInt(count) {
        decodeAssert(Number.isSafeInteger(count) && count >= 0 && count <= 64, "invalid bit width");
        decodeAssert(this.remaining >= count, "unexpected end of delta-of-delta bitstream");
        let value = 0n;
        let remaining = count;
        while (remaining > 0) {
...
class BitWriter {
    ...
    writeBitsNumber(value, count) {
        encodeAssert(Number.isSafeInteger(count) && count >= 0 && count <= 24, "invalid bit width");
    ...
    writeBitsBigInt(value, count) {
        encodeAssert(count >= 0 && count <= 64, "invalid bit width");
        let remaining = count;
        while (remaining > 0) {
            const step = Math.min(remaining, 24);
            remaining -= step;
            this.writeBitsNumber(Number((value >> BigInt(remaining)) & 0xffffffn), step);
        }
    }
    finish() {
        if (this.#bitCount === 0) {
...
function decodeDeltaOfDeltaValue(reader) {
    ...
    if (reader.readBitsNumber(1) === 0) {
        return reader.readBitsNumber(21) - 1048575;
    }
    return BigInt.asIntN(64, reader.readBitsBigInt(64));
}
function encodeDeltaOfDeltaValue(writer, value) {
    if (value === 0n) {
    ...
    else if (value >= -1048575n && value <= 1048576n) {
        writer.writeBitsNumber(0b11110, 5);
        writer.writeBitsNumber(Number(value + 1048575n), 21);
    }
    else {
...
function encodeDeltaOfDeltaValueNumber(writer, value) {
    if (value === 0) {
        writer.writeBitsNumber(0, 1);
    }
    ...
    else if (value >= -1048575 && value <= 1048576) {
        writer.writeBitsNumber(0b11110, 5);
        writer.writeBitsNumber(value + 1048575, 21);
    }
    else {
        writer.writeBitsNumber(0b11111, 5);
...
function checkedI64(value, label) {
    decodeAssert(value >= I64_MIN && value <= I64_MAX, `${label} is out of range`);
    return value;
}
function assertDecodedLength(current, extra, label) {
...
function assertUsize(value) {
    assertBigIntRange(value, 0n, U64_MAX, "usize");
    return value;
}
function assertIsize(value) {
...
function assertBigIntRange(value, min, max, label) {
