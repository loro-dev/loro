...
export function decodeSstable(bytes, options = {}) {
    if (bytes.length === 0) {
        return [];
    }
    decodeAssert(bytes.length >= 17, "SSTable is too short", 0);
    decodeAssert(bytesEqual(bytes.subarray(0, 4), SSTABLE_MAGIC), "invalid SSTable magic", 0);
    ...
    decodeAssert(metadataOffset >= 5 && metadataOffset < bytes.length - 4, "invalid SSTable metadata offset", bytes.length - 4);
    ...
    for (let index = 0; index < metadata.length; index += 1) {
        ...
        const end = index + 1 < metadata.length ? metadata[index + 1].offset : metadataOffset;
        decodeAssert(current.offset >= 5, "invalid SSTable block offset", current.offset);
        decodeAssert(end > current.offset && end <= metadataOffset, "invalid SSTable block range", current.offset);
        ...
        if (options.checkChecksum !== false) {
            decodeAssert(checksum === xxhash32(payload, LORO_XXHASH_SEED), "SSTable block checksum mismatch", end - 4);
        }
        const decoded = current.compression === 0 ? payload : decodeLz4Frame(payload, options);
        if (current.large) {
...
export function encodeSstable(input, options = {}) {
    if (input.length === 0) {
    ...
    for (let index = 0; index < entries.length;) {
        ...
        if (first.value.length > blockSize || first.value.length > 0xffff) {
            blocks.push(encodeStoredBlock(first.value, true, first.key, undefined, compression));
            index += 1;
            continue;
        }
        ...
        blocks.push(encodeStoredBlock(body.toUint8Array(), false, firstKey, lastKey, compression));
    }
    return encodeTable(blocks);
}
function decodeMetadata(bytes, checkChecksum) {
    ...
    if (checkChecksum) {
        ...
        decodeAssert(stored === expected, "SSTable metadata checksum mismatch", bytes.length - 4);
    }
    const metadata = [];
    for (let index = 0; index < count; index += 1) {
        ...
        decodeAssert(compression === 0 || compression === 1, "invalid SSTable compression tag", reader.position - 1);
    ...
    for (let index = 0; index < metadata.length; index += 1) {
        ...
        if (index > 0) {
            decodeAssert(current.offset > metadata[index - 1].offset, "SSTable block offsets are not strictly increasing");
            decodeAssert(compareBytes(metadata[index - 1].firstKey, current.firstKey) < 0, "SSTable metadata keys are not strictly increasing");
        }
    }
    return metadata;
}
function decodeNormalBlock(bytes, metadata) {
    ...
    decodeAssert(metadata.lastKey !== undefined && bytesEqual(last.key, metadata.lastKey), "SSTable metadata last key mismatch");
    return entries;
}
function encodeStoredBlock(raw, large, firstKey, lastKey, compression) {
    const lz4 = compression === "none" ? undefined : encodeLz4FrameRaw(raw);
    const useLz4 = compression === "lz4" || (compression === "auto" && lz4.length <= raw.length);
...
function encodeTable(blocks) {
    encodeAssert(blocks.length > 0 && blocks.length <= MAX_BLOCK_COUNT, "invalid SSTable block count");
    let offset = 5;
    const metadata = [];
    for (const block of blocks) {
        ...
        encodeAssert(offset <= 4294967295, "SSTable exceeds u32 offsets");
    }
    ...
    metadataWriter.writeU32LE(xxhash32(metadataWithoutChecksum.subarray(4), LORO_XXHASH_SEED));
...
function validateEntriesForEncoding(entries) {
...
function validateEntryOrder(entries) {
