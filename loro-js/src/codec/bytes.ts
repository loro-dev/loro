...
export class ByteReader {
  ...
  constructor(bytes: Uint8Array, offset = 0, length = bytes.length - offset) {
  ...
  assertEnd(message = "trailing bytes"): void {
    if (this.remaining !== 0) {
...
export class ByteWriter {
  ...
  constructor(initialCapacity = 64) {
    encodeAssert(
      Number.isSafeInteger(initialCapacity) && initialCapacity >= 0,
      "invalid writer capacity",
    );
...
function assertUnsignedNumber(value: number, max: number, label: string): void {
  if (!Number.isSafeInteger(value) || value < 0 || value > max) {
    throw new LoroEncodeError(`${label} is out of range: ${value}`);
  }
}

function assertUnsignedBigInt(value: bigint, max: bigint, label: string): void {
