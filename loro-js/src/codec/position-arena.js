...
import { decodeAnyRleUsize, decodeColumnarVecMaybeWrapped, encodeAnyRleUsize, encodeColumnarVecWrapped, } from "./serde-columnar";
export function decodePositionArena(bytes) {
    if (bytes.length === 0) {
        return [];
    }
    ...
    const count = readUlebNumber(reader, 10000000);
    ...
    for (const commonBigInt of commonPrefixes) {
        ...
        const suffix = reader.readBytes(readUlebNumber(reader, 2147483647));
...
export function encodePositionArena(positions, options = {}) {
    if (positions.length === 0 && options.encodeEmpty !== true) {
        return new Uint8Array();
    }
    if (positions.length > 10000000) {
