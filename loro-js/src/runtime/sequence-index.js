...
export class SequenceSpan {
    ...
    deletionIdsAt(_offset) {
        return EMPTY_SEQUENCE_IDS;
    }
    retain(_offset, _element) { }
    forEachRetained(_visit) { }
}
...
const LIST_METRICS = { utf16: 1, utf8: 1 };
const listMetrics = () => LIST_METRICS;
// Shared empty list for spans without deletion metadata; callers only read it.
const EMPTY_SEQUENCE_IDS = [];
export class SequenceIndex {
    #root;
    #tail;
    #structureVersion = 0;
    #randomState = 2654435769;
    #metrics;
    #locationsByPeer = new Map();
    #locationsByLamportPeer = new Map();
    #nodesByLocationId = [];
    #deletionsByPeer = new Map();
    #scalarDeletionTargetsByPeer = new Map();
    #deletionRuns = new SequenceDeletionIndex();
    #maxCounterByPeer = new Map();
    #cachedCausalViews = [];
    ...
    get visibleUtf16Length() {
        return visibleMetric(this.#root, "utf16");
    }
    get visibleUtf8Length() {
        ...
        while (node !== undefined || stack.length > 0) {
            ...
            for (let offset = 0; offset < nodeLength(node); offset += 1) {
                if (nodeDeleted(node, offset))
                    continue;
                if (visit(nodeElement(node, offset)) === false)
                    return;
            }
     ...
     */
    forEachVisibleStorageRange(visit) {
        if (this.visibleLength === 0)
            return;
        const stack = [];
        let node = this.#root;
        while (node !== undefined || stack.length > 0) {
            ...
            for (let offset = 0; offset < nodeLength(node); offset += 1) {
                ...
                if (rangeStart >= 0) {
                    if (visit(node.element, rangeStart, offset) === false)
                        return;
                    rangeStart = -1;
                }
            }
    ...
    forEachVisibleRange(start, end, visit) {
    ...
    someVisibleRange(start, end, predicate) {
        let matched = false;
        this.forEachVisibleRange(start, end, (element) => {
    ...
    visibleIdRuns(start, end) {
        const boundedStart = Math.max(0, Math.min(start, this.visibleLength));
        const boundedEnd = Math.max(boundedStart, Math.min(end, this.visibleLength));
        const runs = [];
        const append = (id, length = 1) => {
            if (length === 0)
                return;
    ...
    visibleMetricRangesForIdRuns(runs, metric) {
    ...
    containsIdRuns(runs) {
        if (runs.length === 0)
            return true;
        const targets = counterRangesByPeer(runs);
        const expected = [...targets.values()].reduce((total, ranges) => total + ranges.reduce((subtotal, range) => subtotal + range.end - range.start, 0), 0);
    ...
    findNextIncludedPhysical(start, version) {
    ...
    visibleInsertionContext(position) {
    ...
    visibleInsertionIdContext(position, output) {
        if (!Number.isSafeInteger(position) ||
            position < 0 ||
            position > this.visibleLength) {
            throw new RangeError(`visible sequence position ${position} is out of range`);
    ...
    findByLamport(peer, lamport) {
        let lamports = this.#locationsByLamportPeer.get(peer);
        if (lamports === undefined) {
            const locations = this.#locationsByPeer.get(peer);
            if (locations === undefined)
                return undefined;
            const indexedLamports = new Map();
            locations.forEach(0, Number.MAX_SAFE_INTEGER, (location) => {
                const lamport = typeof location === "number"
                    ? nodeLamport(this.#nodeAtLocation(location), location % SEQUENCE_LOCATION_STRIDE)
                    : location.lamport;
        ...
        const element = location === undefined
            ? undefined
            : typeof location === "number"
                ? this.#elementAtLocation(location)
                : location;
    ...
    visibleMetricOffsetOf(element, metric) {
        if (this.#hasLazyDeletions)
            materializeElementDeletion(element, this.#metrics);
        const node = elementNode(element);
        const elementPhysicalOffset = elementOffset(element);
        if (node === undefined || elementPhysicalOffset === undefined)
            return undefined;
        let offset = visibleMetric(node.left, metric) +
    ...
    metricOffsetAtVisibleIndex(index, metric) {
        if (!Number.isSafeInteger(index) || index < 0 || index > this.visibleLength) {
            return undefined;
        }
        if (index === this.visibleLength)
            return visibleMetric(this.#root, metric);
        let node = this.#root;
        let remaining = index;
        let offset = 0;
        while (node !== undefined) {
    ...
    visibleIndexAtMetricOffset(offset, metric) {
    ...
    insertAtPhysical(position, elements) {
            ...
            if (idsAreOrdered) {
                ...
                for (const element of elements) {
                    ...
                    if (ids.has(key) ||
                        ...
                        throw new Error(`duplicate sequence id ${element.id.counter}@${element.id.peer.toString()}`);
                    }
                    ids.add(key);
                }
            }
        ...
        if (elements.length <= MAX_SEQUENCE_SPAN) {
            ...
            for (let start = 0; start < elements.length; start += MAX_SEQUENCE_SPAN) {
                insertedRoot = merge(insertedRoot, this.#newNode(elements.slice(start, start + MAX_SEQUENCE_SPAN), true), this.#metrics);
            }
        }
        const appendAtTail = position === this.allLength;
        const previousTailElement = appendAtTail || this.#tail === undefined
            ? undefined
            : nodeElement(this.#tail, nodeLength(this.#tail) - 1);
        if (appendAtTail) {
            this.#root = this.#appendAtTail(this.#root, insertedRoot);
        }
        else if (insertedRoot !== undefined &&
    ...
    insertSpanAtPhysical(position, span) {
        if (span.length === 0)
            return;
        if (position < 0 || position > this.allLength) {
            throw new RangeError(`physical sequence position ${position} is out of range`);
        ...
        if (span.length <= MAX_SEQUENCE_SPAN) {
            ...
            for (let start = 0; start < span.length; start += MAX_SEQUENCE_SPAN) {
                insertedRoot = merge(insertedRoot, this.#newNode(span.slice(start, Math.min(span.length, start + MAX_SEQUENCE_SPAN)), true), this.#metrics);
            }
        }
        const appendAtTail = position === this.allLength;
        const previousTailElement = appendAtTail || this.#tail === undefined
    ...
    setDeleted(element, deleted) {
        if (this.#hasLazyDeletions)
            materializeElementDeletion(element, this.#metrics);
        const node = elementNode(element);
        const offset = elementOffset(element);
        if (node === undefined || offset === undefined || element.deleted === deleted)
            return;
        this.#invalidateCausalView();
        updateNodeElementVisibility(node, offset, element, deleted, this.#metrics);
        recomputeToRoot(node, this.#metrics, false);
    }
    deleteElement(element, deletedBy) {
        if (this.#hasLazyDeletions)
            materializeElementDeletion(element, this.#metrics);
        const node = elementNode(element);
        if (node === undefined) {
        ...
        if (element.deleted && deletedBy === undefined)
            return;
        this.#invalidateCausalView();
        if (!element.deleted) {
            updateNodeElementVisibility(node, elementOffset(element), element, true, this.#metrics);
            recomputeToRoot(node, this.#metrics, false);
        }
        ...
        if (element.deletedByPeer === undefined && (element.deletedBy?.length ?? 0) === 0) {
            ...
            (element.deletedBy ??= []).push(deletedBy);
    ...
    addDeletion(element, id) {
        ...
        if (element.deletedByPeer === undefined && (element.deletedBy?.length ?? 0) === 0) {
            ...
            (element.deletedBy ??= []).push(id);
        this.#recordDeletionElement(element, id);
        this.recordOperationId(id);
    }
    deleteIdSpan(startId, length, deletedBy) {
        ...
        const result = deleteByTree
            ? markIdRunsDeleted(this.#root, singleCounterRangeRelation(startId.peer, startId.counter, startId.counter + spanLength), this.#metrics)
            : this.#markLocatedIdSpanVisibility(startId, spanLength, true);
        if (!result.found)
            return;
        if (deleteByTree)
            this.#hasLazyDeletions = true;
        this.#invalidateCausalView();
        if (deletedBy !== undefined) {
    ...
    #markLocatedIdSpanVisibility(startId, length, deleted) {
        ...
        if (targetNodes.length <= MAX_SEQUENCE_SPAN) {
            ...
            for (const node of targetNodes) {
                if (materializedNodes.includes(node))
                    continue;
            ...
            for (let index = 0; index < targetNodes.length; index += 1) {
                ...
                if (nodeDeleted(node, targetOffsets[index]) === deleted)
                    continue;
    ...
    setIdRunsDeleted(runs) {
        if (runs.length === 0)
            return;
        const run = runs.length === 1 ? runs[0] : undefined;
        const targetedLimit = Math.max(64, Math.min(512, this.allLength >>> 8));
        const updateByTree = run === undefined || this.#root?.idRunCount === 1 || run.length > targetedLimit;
        const result = updateByTree
            ? markIdRunsDeleted(this.#root, run === undefined
                ? mapCounterRangeRelation(counterRangesByPeer(runs))
                : singleCounterRangeRelation(run.start.peer, run.start.counter, run.start.counter + run.length), this.#metrics)
    ...
    setIdRunsVisible(runs) {
        ...
        const updateByTree = run === undefined || this.#root?.idRunCount === 1 || run.length > targetedLimit;
        const result = updateByTree
            ? markIdRunsVisible(this.#root, run === undefined
                ? mapCounterRangeRelation(counterRangesByPeer(runs))
                : singleCounterRangeRelation(run.start.peer, run.start.counter, run.start.counter + run.length), this.#metrics)
    ...
    canShowIdRunsAt(runs, version) {
        for (const run of runs) {
            if (run.start.counter + run.length > (version.get(run.start.peer) ?? 0)) {
                return false;
            }
        }
        if (this.#deletionRuns.hasDeletionAt(runs, version))
            return false;
        for (const run of runs) {
            if (this.#scalarDeletionTargetsByPeer
                .get(run.start.peer)
                ?.some(run.start.counter, run.start.counter + run.length, (location) => someSequenceDeletion(this.#elementAtDeletionLocation(location), (id) => id.counter < (version.get(id.peer) ?? 0)))) {
                return false;
            }
        }
        return true;
    }
    elementsDeletedBy(peer, start, end) {
        ...
        for (const run of this.#deletionRuns.targetRunsDeletedBy(peer, start, end)) {
            for (let offset = 0; offset < run.length; offset += 1) {
                ...
                if (location !== undefined) {
                    selected.add(typeof location === "number"
                        ? {
        ...
        this.#deletionsByPeer.get(peer)?.forEach(start, end, (elementOrSet) => {
            if (elementOrSet instanceof Set) {
                for (const element of elementOrSet)
    ...
    idRunsDeletedBy(peer, start, end) {
    ...
    someDeletion(element, predicate) {
        return (this.#deletionRuns.someDeletion(element.id, predicate) ||
            someSequenceDeletion(element, predicate));
    }
    everyDeletion(element, predicate) {
        return (this.#deletionRuns.everyDeletion(element.id, predicate) &&
            everySequenceDeletion(element, predicate));
    }
    moveVisible(from, to) {
        const element = this.atVisible(from);
        if (element === undefined || from === to)
            return;
        this.#invalidateCausalView();
        ...
        const destination = destinationElement === undefined
            ? this.allLength
    ...
    moveBefore(element, before) {
        ...
        const destination = before === undefined ? this.allLength : this.physicalIndexOf(before);
        if (destination === undefined) {
            ...
            this.#root = merge(merge(beforeDestination, selected, this.#metrics), afterDestination, this.#metrics);
        }
        if (this.#root !== undefined)
            this.#root.parent = undefined;
        this.#tail = this.#root === undefined ? undefined : rightmost(this.#root);
        ...
        while (current.parent !== undefined) {
            ...
            if (current === parent.left) {
                for (let index = 0; index < nodeLength(parent); index += 1) {
                    if (!nodeDeleted(parent, index))
                        return nodeElement(parent, index);
                }
    ...
    isFullyIncluded(version) {
        for (const [peer, counter] of this.#maxCounterByPeer) {
            if ((version.get(peer) ?? 0) < counter)
                return false;
        }
        return true;
    }
    causalVisible(version) {
    ...
    causalView(version) {
        ...
        const cachedIndex = this.#cachedCausalViews.findIndex((cached) => versionsEqual(cached.version, version));
        if (cachedIndex >= 0) {
        ...
        const included = (id) => id.counter < (causalVersion.get(id.peer) ?? 0);
        const deletionIncluded = (deleteId) => !included(deleteId);
        ...
        const causalCounts = new Map();
        const causalCount = (node) => {
            if (node === undefined)
                return 0;
            ...
            if (node.idRunCount === 1) {
                ...
                if (includedEnd > node.lastId.counter &&
                    this.canShowIdRunsAt([{ start: node.firstId, length: node.allCount }], causalVersion)) {
        ...
        const view = {
                ...
                while (node !== undefined) {
                    ...
                    for (let offset = 0; offset < nodeLength(node); offset += 1) {
                        if (!isVisible(node, offset))
                            continue;
                        if (remaining === 0)
                            return nodeElement(node, offset);
                        remaining -= 1;
                    }
                    node = node.right;
                }
            ...
            range: (start, end) => {
                ...
                const output = [];
                collectCausalRange(this.#root, boundedStart, boundedEnd, output, causalCount, isVisible, this.#metrics);
                return output;
            },
            idRuns: (start, end) => {
                const boundedStart = Math.max(0, Math.min(start, length));
                const boundedEnd = Math.max(boundedStart, Math.min(end, length));
                const output = [];
                collectCausalIdRuns(this.#root, boundedStart, boundedEnd, output, causalCount, isVisible);
                return output;
            },
        };
    ...
    reservePeerCounters(peer, endExclusive) {
    ...
    #newNode(storage, recordNew) {
        ...
        if (recordNew)
            this.#recordNewElements(node);
        else
            this.#assignLocations(node);
        return node;
    }
    #recordNewElements(node) {
        for (let offset = 0; offset < nodeLength(node); offset += 1) {
            this.#recordNewElement(node, offset);
        }
    }
    #recordNewElement(node, offset) {
        if (!node.isSpan) {
        ...
        const deletionIds = node.element.deletionIdsAt(offset);
    ...
    #recordNewPlainElement(node, offset, element) {
    ...
    #assignLocations(node) {
        if (node.isSpan) {
            node.element.forEachRetained((element, offset) => {
                setElementLocation(element, node, offset);
            });
        }
        else {
    ...
    #nodeAtLocation(location) {
        ...
        if (node === undefined)
            throw new Error("sequence location points to a missing node");
        return node;
    }
    #elementAtLocation(location) {
        return nodeElement(this.#nodeAtLocation(location), location % SEQUENCE_LOCATION_STRIDE);
    }
    #elementAtDeletionLocation(location) {
        if ("stableSequenceId" in location) {
            const element = this.findById(location);
            if (element === undefined)
                throw new Error("deleted sequence element is missing");
            return element;
        }
        return location;
    }
    #recordDeletionElement(element, id) {
        const node = elementNode(element);
        const offset = elementOffset(element);
        const location = node?.isSpan === true && offset !== undefined
            ? {
                stableSequenceId: true,
    ...
    #split(root, count) {
        ...
        if (cut !== undefined) {
            if (remaining === cutLeftCount) {
                ...
                const rightRoot = this.#newNode(sliceNodeStorage(storage, ownOffset, nodeLengthFromStorage(storage)), false);
    ...
    #appendToPreviousNode(position, elements) {
        if (position === 0)
            return false;
    ...
    #appendSpanToPreviousNode(position, span) {
        if (position === 0)
            return false;
        const previous = this.atPhysical(position - 1);
        if (previous === undefined)
        ...
        for (let insertedOffset = 0; insertedOffset < span.length; insertedOffset += 1) {
            ...
            if (!nodeDeleted(node, offsetInNode)) {
                ...
                else if (!sequenceIdScalarsContinue(lastVisiblePeer, lastVisibleCounter, peer, counter)) {
                    node.ownVisibleIdRunCount += 1;
                }
    ...
    #insertSingleNode(root, position, inserted) {
        if (root === undefined) {
 ...
 */
function everyNodeElementDeletion(node, offset, predicate) {
    if (node.isSpan) {
        const deletionIds = node.element.deletionIdsAt(offset);
        for (let index = 0; index < deletionIds.length; index += 1) {
            if (!predicate(deletionIds[index]))
                return false;
        }
        return true;
    }
    const storage = node.element;
    return everySequenceDeletion(Array.isArray(storage) ? storage[offset] : storage, predicate);
}
function elementNode(element) {
    return element[SEQUENCE_NODE];
}
function elementOffset(element) {
    return element[SEQUENCE_OFFSET];
}
function setElementLocation(element, node, offset) {
...
function nodeLengthFromStorage(storage) {
...
function nodeElement(node, offset) {
    if (node.isSpan) {
        const element = node.element.elementAt(offset);
        setElementLocation(element, node, offset);
        return element;
    }
    const storage = node.element;
...
function nodePeer(node, offset) {
    return node.isSpan
        ? node.element.peerAt(offset)
        : Array.isArray(node.element)
            ? node.element[offset].id.peer
            : node.element.id.peer;
}
function nodeCounter(node, offset) {
    return node.isSpan
        ? node.element.counterAt(offset)
        : Array.isArray(node.element)
            ? node.element[offset].id.counter
            : node.element.id.counter;
}
function findNextIncludedPhysical(node, nodeStart, start, version) {
    if (node === undefined || nodeStart + node.allCount <= start)
        return undefined;
    if (node.idRunCount === 1 &&
    ...
    return findNextIncludedPhysical(node.right, ownStart + nodeLength(node), start, version);
}
function nodeLamport(node, offset) {
    return node.isSpan
        ? node.element.lamportAt(offset)
        : Array.isArray(node.element)
            ? node.element[offset].lamport
            : node.element.lamport;
}
function nodeDeleted(node, offset) {
    return node.isSpan
        ? node.element.deletedAt(offset)
        : Array.isArray(node.element)
            ? node.element[offset].deleted
            : node.element.deleted;
}
function setNodeDeleted(node, offset, deleted) {
    if (node.isSpan) {
        node.element.setDeletedAt(offset, deleted);
    }
    else if (Array.isArray(node.element)) {
        node.element[offset].deleted = deleted;
    }
    else {
        node.element.deleted = deleted;
    }
}
function nodeMetrics(node, offset, metrics) {
    return node.isSpan
        ? node.element.metricsAt(offset)
        : metrics(Array.isArray(node.element)
            ? node.element[offset]
            : node.element);
}
function sliceNodeStorage(storage, start, end) {
...
function appendNodeElements(node, output) {
    if (node.isSpan) {
...
function allCount(node) {
    return node?.allCount ?? 0;
}
function visibleCount(node) {
    return node?.visibleCount ?? 0;
}
function visibleMetric(node, metric) {
    if (node === undefined)
        return 0;
    return metric === "utf16" ? node.visibleUtf16 : node.visibleUtf8;
}
function ownVisibleCount(node) {
    return node.ownVisibleCount;
}
function visibleElementsBefore(node, offset) {
    if (node.visibleOffsets !== undefined)
        return node.visibleOffsets[offset];
    let count = 0;
...
function ownVisibleMetric(node, metric, _metrics) {
    return metric === "utf16" ? node.ownVisibleUtf16 : node.ownVisibleUtf8;
}
function visibleMetricBefore(node, offset, metric, metrics) {
    ...
    for (let index = 0; index < offset; index += 1) {
        if (!nodeDeleted(node, index))
            total += nodeMetrics(node, index, metrics)[metric];
    }
    return total;
}
function physicalOffsetAtVisibleIndex(node, index) {
    if (node.ownVisibleCount === node.ownCount)
        return index;
...
function physicalOffsetAtMetricOffset(node, offset, metric) {
    const prefix = metric === "utf16" ? node.visibleUtf16Prefix : node.visibleUtf8Prefix;
    if (prefix === undefined) {
        if (offset === 0)
            return 0;
...
function updateNodeElementVisibility(node, offset, element, deleted, metrics) {
...
function recomputeOwn(node, metrics) {
    if (!node.isSpan) {
...
function recomputeOwnElements(node, metrics) {
    const storage = node.element;
    const elements = Array.isArray(storage) ? storage : undefined;
    const first = elements?.[0] ?? storage;
    const last = elements?.at(-1) ?? storage;
    ...
    if (elements === undefined) {
        ...
        if (storage.deleted) {
            ...
            node.ownVisibleUtf16 = value.utf16;
            node.ownVisibleUtf8 = value.utf8;
            node.ownFirstVisibleId = storage.id;
            node.ownLastVisibleId = storage.id;
            node.ownVisibleIdRunCount = 1;
        }
        return;
    }
...
function recomputeVisibility(node) {
...
function recompute(node, _metrics) {
    const left = node.left;
    const right = node.right;
    if (left === undefined && right === undefined) {
...
function applyNodeDeleted(node, metrics) {
    if (node.lazyDeleted && node.visibleCount === 0)
        return;
    for (let offset = 0; offset < nodeLength(node); offset += 1) {
...
function applyNodeVisible(node, metrics) {
    if (node.lazyVisible && node.visibleCount === node.allCount)
        return;
    for (let offset = 0; offset < nodeLength(node); offset += 1) {
...
function pushNodeDeletion(node, metrics) {
    if (!node.lazyDeleted && !node.lazyVisible)
        return;
    if (node.left !== undefined) {
        if (node.lazyDeleted)
            applyNodeDeleted(node.left, metrics);
        else
            applyNodeVisible(node.left, metrics);
    }
    if (node.right !== undefined) {
        if (node.lazyDeleted)
            applyNodeDeleted(node.right, metrics);
        else
            applyNodeVisible(node.right, metrics);
    }
    node.lazyDeleted = false;
    node.lazyVisible = false;
}
function materializeElementDeletion(element, metrics) {
    const node = elementNode(element);
    if (node === undefined)
        return;
    materializeNodeDeletion(node, metrics);
}
function materializeNodeDeletion(node, metrics) {
    const path = [];
    let current = node;
    while (current !== undefined) {
        path.push(current);
...
function materializeAllDeletions(node, metrics) {
    if (node === undefined)
        return;
...
function recomputeOwnIdRuns(node) {
    node.ownFirstVisibleId = undefined;
    node.ownLastVisibleId = undefined;
    node.ownVisibleIdRunCount = 0;
    let lastPeer;
...
function sequenceIdsContinue(left, right) {
    return left.peer === right.peer && left.counter + 1 === right.counter;
}
function sequenceIdScalarsContinue(leftPeer, leftCounter, rightPeer, rightCounter) {
    return leftPeer === rightPeer && leftCounter + 1 === rightCounter;
}
function normalizeSequenceIdRuns(runs) {
...
function recomputeToRoot(node, metrics, includePhysical = true) {
    let current = node;
    while (current !== undefined) {
        if (includePhysical)
...
function recomputeAffected(node, affected, metrics) {
    if (node === undefined || !affected.has(node))
        return;
...
function visitInOrder(root, visit) {
    const stack = [];
    let node = root;
    while (node !== undefined || stack.length > 0) {
        while (node !== undefined) {
...
function visitVisibleIdRuns(node, start, end, metrics, visit) {
    if (node === undefined || start >= end)
        return;
    ...
    if (end > rightStart) {
        visitVisibleIdRuns(node.right, Math.max(0, start - rightStart), end - rightStart, metrics, visit);
    }
}
function counterRangesByPeer(runs) {
...
function counterRangeRelation(targets, peer, start, end) {
    ...
    if (range === undefined || range.start >= end)
        return 0;
    return range.start <= start && range.end >= end ? 1 : -1;
}
function mapCounterRangeRelation(targets) {
 ...
 */
function singleCounterRangeRelation(targetPeer, targetStart, targetEnd) {
    return (peer, start, end) => {
        if (peer !== targetPeer || targetStart >= end || targetEnd <= start)
            return 0;
        return targetStart <= start && targetEnd >= end ? 1 : -1;
    };
}
function visitVisibleMetricRangesForIds(node, base, metric, targets, metrics, visit) {
    if (node === undefined || node.visibleCount === 0)
        return;
    if (node.visibleIdRunCount === 1 && node.firstVisibleId !== undefined) {
        const relation = counterRangeRelation(targets, node.firstVisibleId.peer, node.firstVisibleId.counter, node.firstVisibleId.counter + node.visibleCount);
        if (relation === 0)
            return;
        if (relation === 1) {
    ...
    for (let offset = 0; offset < nodeLength(node); offset += 1) {
        ...
        const counter = nodeCounter(node, offset);
        if (counterRangeRelation(targets, nodePeer(node, offset), counter, counter + 1) === 1) {
...
function countPhysicalIdsInRanges(node, targets) {
    if (node === undefined)
        return 0;
    if (node.idRunCount === 1) {
        const relation = counterRangeRelation(targets, node.firstId.peer, node.firstId.counter, node.firstId.counter + node.allCount);
        if (relation === 0)
            return 0;
        if (relation === 1)
            return node.allCount;
...
function markIdRunsDeleted(node, relate, metrics) {
    if (node === undefined)
        return { found: false, changed: false };
    if (node.idRunCount === 1) {
        const relation = relate(node.firstId.peer, node.firstId.counter, node.firstId.counter + node.allCount);
        if (relation === 0)
            return { found: false, changed: false };
        if (relation === 1) {
...
function markIdRunsVisible(node, relate, metrics) {
    if (node === undefined)
        return { found: false, changed: false };
    if (node.idRunCount === 1) {
        const relation = relate(node.firstId.peer, node.firstId.counter, node.firstId.counter + node.allCount);
...
function visitVisibleRange(node, start, end, metrics, visit) {
    if (node === undefined || start >= end)
        return true;
    pushNodeDeletion(node, metrics);
    const leftCount = visibleCount(node.left);
    ...
    if (end > ownOffset) {
        return visitVisibleRange(node.right, Math.max(0, start - ownOffset), end - ownOffset, metrics, visit);
    }
    return true;
}
function collectCausalRange(node, start, end, output, count, isVisible, metrics) {
    if (node === undefined || start >= end)
        return;
    pushNodeDeletion(node, metrics);
    const leftCount = count(node.left);
    if (start < leftCount) {
        collectCausalRange(node.left, start, Math.min(end, leftCount), output, count, isVisible, metrics);
    }
    let ownOffset = leftCount;
    for (let offset = 0; offset < nodeLength(node); offset += 1) {
        if (!isVisible(node, offset))
    ...
    if (end > ownOffset) {
        collectCausalRange(node.right, Math.max(0, start - ownOffset), end - ownOffset, output, count, isVisible, metrics);
    }
}
function collectCausalIdRuns(node, start, end, output, count, isVisible) {
    if (node === undefined || start >= end)
        return;
    ...
    const leftCount = count(node.left);
    if (start < leftCount) {
        collectCausalIdRuns(node.left, start, Math.min(end, leftCount), output, count, isVisible);
    }
    let ownOffset = leftCount;
    for (let offset = 0; offset < nodeLength(node); offset += 1) {
        if (!isVisible(node, offset))
    ...
    if (end > ownOffset) {
        collectCausalIdRuns(node.right, Math.max(0, start - ownOffset), end - ownOffset, output, count, isVisible);
    }
}
function appendSequenceIdRun(output, start, length) {
    if (length <= 0)
        return;
    const previous = output.at(-1);
    if (previous !== undefined &&
...
function firstPhysicalElement(node) {
    if (node === undefined)
        return undefined;
...
function writeFirstPhysicalId(node, output) {
    if (node === undefined) {
...
function nextPhysicalElement(node, offset) {
    if (offset + 1 < nodeLength(node))
        return nodeElement(node, offset + 1);
    ...
    while (current.parent !== undefined) {
        if (current === current.parent.left)
            return nodeElement(current.parent, 0);
        current = current.parent;
    }
    return undefined;
}
function writeNextPhysicalId(node, offset, output) {
    if (offset + 1 < nodeLength(node)) {
...
function firstVisibleElement(node, metrics) {
...
function lastVisibleElement(node, metrics) {
    while (node !== undefined && node.visibleCount > 0) {
        pushNodeDeletion(node, metrics);
...
function versionsEqual(left, right) {
    ...
    for (const [peer, counter] of left) {
        if (right.get(peer) !== counter)
            return false;
    }
    return true;
}
const MAX_COUNTER_DENSE_GAP = 4096;
const COUNTER_PAGE_SIZE = 1024;
class PagedCounterStore {
    ...
    some(start, end, predicate) {
        if (start >= end)
            return false;
        const firstPage = (start / COUNTER_PAGE_SIZE) | 0;
        const lastPage = ((end - 1) / COUNTER_PAGE_SIZE) | 0;
        for (let index = lowerBoundNumber(this.#pageIndexes, firstPage); index < this.#pageIndexes.length && this.#pageIndexes[index] <= lastPage; index += 1) {
...
class CounterStore {
    #dense = [];
    #sparse = new Map();
    #sparseCounters = new CounterIndex();
    get(counter) {
    ...
    set(counter, value) {
        if (counter < 4294967295 && counter <= this.#dense.length + MAX_COUNTER_DENSE_GAP) {
            this.#dense[counter] = value;
            return;
        }
        if (!this.#sparse.has(counter))
            this.#sparseCounters.add(counter);
        this.#sparse.set(counter, value);
    }
    reserveDense(endExclusive) {
        if (this.#sparse.size > 0) {
            throw new Error("cannot reserve dense sequence counters after sparse insertion");
        }
        if (endExclusive > this.#dense.length)
            this.#dense.length = endExclusive;
    }
    forEach(start, end, visit) {
        if (start >= end)
            return;
...
class CounterIndex {
    #ranges = [];
    #singletons = [];
    #outOfOrder = new CounterRangeTree();
    #lastCounter;
    #tailIsRange = false;
    add(counter) {
    ...
    forEach(start, end, visit) {
        ...
        if (index > 0) {
            ...
            if (previousEnd > start) {
                visitCounterInterval(this.#ranges[(index - 1) * 2], previousEnd, start, end, visit);
            }
        }
        for (let rangeIndex = index; rangeIndex < this.#ranges.length / 2 && this.#ranges[rangeIndex * 2] < end; rangeIndex += 1) {
            visitCounterInterval(this.#ranges[rangeIndex * 2], this.#ranges[rangeIndex * 2 + 1], start, end, visit);
        }
        for (let singletonIndex = lowerBoundNumber(this.#singletons, start); singletonIndex < this.#singletons.length && this.#singletons[singletonIndex] < end; singletonIndex += 1) {
...
class CounterRangeTree {
    #ranges = new OrderedIndex((left, right) => left.start - right.start);
    add(counter) {
...
function lowerBoundCounterRange(ranges, counter) {
    let low = 0;
    let high = ranges.length / 2;
    while (low < high) {
        const middle = (low + high) >>> 1;
        if (ranges[middle * 2] < counter)
            low = middle + 1;
        else
            high = middle;
    }
...
function lowerBoundNumber(values, value) {
    let low = 0;
...
function containsCounter(ranges, singletons, counter) {
    ...
    if (rangeIndex * 2 < ranges.length && ranges[rangeIndex * 2] === counter)
        return true;
    return rangeIndex > 0 && counter < ranges[rangeIndex * 2 - 1];
}
function visitCounterInterval(rangeStart, rangeEnd, start, end, visit) {
    const boundedEnd = Math.min(rangeEnd, end);
    for (let counter = Math.max(rangeStart, start); counter < boundedEnd; counter += 1) {
        visit(counter);
    }
}
function visitCounterRange(range, start, end, visit) {
