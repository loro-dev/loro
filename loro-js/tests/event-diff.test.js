...
describe("incremental event diffs", () => {
    test("composes randomized text edits against a string model", () => {
        ...
        expect(applyTextDelta(original, result.diff)).toBe(current);
    });
    test("composes randomized list edits against an array model", () => {
        ...
        expect(applyListDelta(original, result.diff)).toEqual(current);
    });
    ...
    test("emits operation-composed local diffs for every indexed container", () => {
        ...
        target.applyDiff(batch.events.map(({ target: id, diff: eventDiff }) => [id, eventDiff]));
    ...
    test("records imported concurrent edits at their actual visible positions", () => {
        ...
        right.import(update);
        expect(batch).toBeDefined();
        mirror.applyDiff(batch.events.map(({ target: id, diff: eventDiff }) => [id, eventDiff]));
...
function applyListDelta(original, delta) {
    let cursor = 0;
    const output = [];
    for (const operation of delta) {
        if ("retain" in operation) {
