...
import { ContainerType, decodeChangeBlock, encodeChangeBlock, validateChangeBlock, } from "../src/codec/index";
describe("semantic change block codec", () => {
    test("round trips a map insert block", () => {
        ...
        const block = {
            peers: [0x0102030405060708n],
            keys: ["root", "a"],
            containers: [container],
            positions: [],
            changes: [
                {
                    id: { peer: 0x0102030405060708n, counter: 10 },
                    timestamp: 1234n,
                    dependencies: [],
                    lamport: 100,
                    message: undefined,
    ...
    test("round trips deletes, movable-list operations and tree operations", () => {
        ...
        expect(decoded.peers).toEqual(expect.arrayContaining([peer, 0xffffffffffffffffn]));
        expect(validateChangeBlock(encoded)).toEqual({
            peer,
            counterStart: 0,
            counterEnd: 5,
