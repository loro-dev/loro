...
describe("LZ4 frame", () => {
  ...
  test.each([0, 1, 65_536, 65_537, 300_000])(
    "round trips %i canonical frame bytes",
    (length) => {
      ...
      expect(
        decodeLz4Frame(encodeLz4FrameRaw(input), {
  ...
  test("encodes an overlapping match with the required trailing literals", () => {
    ...
    expect(bytesToHex(encoded)).toBe(
      "04224d186040820a0000001861010050616161616100000000",
    );
  ...
  test("compresses highly repetitive blocks", () => {
    ...
    const encoded = encodeLz4FrameRaw(input);
    expect(readFirstBlockInfo(encoded) & 0x8000_0000).toBe(0);
  ...
  test("stores incompressible blocks without expansion", () => {
    ...
    const encoded = encodeLz4FrameRaw(input);
    expect(readFirstBlockInfo(encoded) & 0x8000_0000).not.toBe(0);
  ...
  test("round trips mixed noisy and repeated ranges", () => {
    for (const [caseIndex, length] of [
      13, 14, 15, 31, 255, 256, 4095, 65_535, 65_536, 65_537,
    ].entries()) {
      const input = deterministicNoise(length, 0x1234_5678 + caseIndex);
...
function readFirstBlockInfo(frame: Uint8Array): number {
  return new DataView(frame.buffer, frame.byteOffset + 7, 4).getUint32(0, true);
}

function deterministicNoise(length: number, seed = 0x1234_5678): Uint8Array {
