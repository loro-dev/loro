...
import { ContainerType, decodeFastSnapshot, decodePostcardVersionVector, decodeStateSnapshotStore, decodeSstable, encodeFastSnapshot, encodePostcardVersionVector, encodeStateSnapshotStore, encodeSstable, } from "../src/codec";
import { Awareness, AwarenessWasm, Cursor, decodeFrontiers, decodeImportBlobMeta, encodeFrontiers, EphemeralStore, EphemeralStoreWasm, LORO_VERSION, LoroCounter, LoroDoc, LoroList, LoroMap, LoroMovableList, LoroText, redactJsonUpdates, UndoManager, VersionVector, } from "../src/index";
const fixture = (name) => new Uint8Array(readFileSync(new URL(`./fixtures/rust/${name}`, import.meta.url)));
const withDuplicateTextId = (snapshot) => {
    const stateStore = decodeStateSnapshotStore(snapshot.state);
    if (stateStore.kind !== "sstable")
        throw new Error("expected snapshot state");
    let changed = false;
    const containers = stateStore.containers.map((entry) => {
        const state = entry.wrapper.state;
        if (changed || state.kind !== ContainerType.Text)
            return entry;
        ...
        return {
            ...
            wrapper: {
                ...
                state: {
                    ...
                    spans: state.spans.flatMap((item) => item === span
                        ? [
                            { ...span, length: 1 },
    ...
    if (!changed)
        throw new Error("expected a splittable text span");
    return encodeFastSnapshot({
        ...snapshot,
        state: encodeStateSnapshotStore({ ...stateStore, containers }, { compression: "none" }),
    });
};
describe("loro-wasm-compatible runtime", () => {
    ...
    test("evaluates JSONPath selectors, recursion, slices, and filters", () => {
        ...
        expect(doc.JSONPath("$.store.books[?(@.author == $.store.featured_author && @.price < 10)].title")).toEqual(["Animal Farm"]);
        expect(doc.JSONPath("$.store.books[?(@.author in $.store.featured_authors)].title")).toEqual(["1984", "Animal Farm", "Pride and Prejudice"]);
    ...
    test("queries version spans, ancestors and changed containers", () => {
        ...
        expect(new Set(doc.getChangedContainersIn({ peer: "1", counter: 5 }, 7))).toEqual(new Set([doc.getText("text").id, doc.getMap("map").id]));
        const messages = [];
    ...
    test("applies raw and serialized diffs with fresh child and tree ids", () => {
        ...
        const serialized = JSON.parse(JSON.stringify(source.diff([], source.frontiers())));
        for (const batch of [raw, serialized]) {
            const target = new LoroDoc();
            target.setPeerId(2);
            target.applyDiff(batch);
            const targetChild = target.getMap("map").get("child");
    ...
    test("reverts by generating operations, including recreated child containers", () => {
        ...
        expect(list.get(2).id).not.toBe(child.id);
        doc.commit();
        doc.revertTo([]);
    ...
    test("round-trips compressed and uncompressed JSON updates", () => {
        ...
        expect(source.getOpsInChange({ peer: "19", counter: 0 })).toEqual(source.exportJsonInIdSpan({
            peer: "19",
            counter: 0,
    ...
    test("decodes update, snapshot, and shallow-snapshot blob metadata", () => {
        ...
        const shallow = decodeImportBlobMeta(doc.export({ mode: "shallow-snapshot", frontiers: firstFrontiers }));
    ...
    test("coalesces consecutive list inserts in one pending change", () => {
        ...
        const target = new LoroDoc();
        target.import(source.export({ mode: "update" }));
        expect(target.getList("list").toArray()).toEqual(Array.from({ length: 128 }, (_, index) => index));
    });
    ...
    test("emits absolute paths and bubbles descendant events to container subscribers", () => {
        ...
        subMap.set("value", 1);
        doc.commit();
        expect(seen.at(-1)?.events.find((event) => event.target === subMap.id)?.path).toEqual(["map", "sub"]);
    ...
    test("coalesces consecutive text IDs without splitting Unicode scalars", () => {
        ...
        if (stateStore.kind !== "sstable")
            throw new Error("expected snapshot state");
        const textState = stateStore.containers.find(({ wrapper }) => wrapper.state.kind === ContainerType.Text)?.wrapper.state;
        expect(textState?.kind).toBe(ContainerType.Text);
        if (textState?.kind !== ContainerType.Text)
            throw new Error("expected text state");
    ...
    test("bulk-loads multi-span text snapshots and remains editable", () => {
        ...
        sourceText.insert(0, Array.from({ length: 80 }, (_, index) => `${index % 10}😀`).join(""));
    ...
    test("rejects duplicate text IDs before changing the document", () => {
        ...
        source.commit();
        const malformed = withDuplicateTextId(decodeFastSnapshot(source.export({ mode: "snapshot" })));
        const target = new LoroDoc();
        const events = [];
    ...
    test("rejects duplicate text IDs before installing shallow metadata", () => {
        ...
        source.commit();
        const malformed = withDuplicateTextId(decodeFastSnapshot(source.export({ mode: "shallow-snapshot", frontiers: shallowRoot })));
        const target = new LoroDoc();
        const events = [];
        target.subscribe((event) => events.push(event));
        expect(() => target.import(malformed)).toThrow(/duplicate sequence IDs/u);
        ...
        expect(() => batchTarget.importBatch([malformed, concurrent.export({ mode: "update" })])).toThrow(/duplicate sequence IDs/u);
    ...
    test("rejects a corrupt deferred frontier block before changing the document", () => {
        ...
        source.commit();
        const snapshot = decodeFastSnapshot(source.export({ mode: "snapshot" }));
        const oplog = decodeSstable(snapshot.oplog).map((entry) => entry.key.length === 12 ? { ...entry, value: Uint8Array.of(0xff) } : entry);
        const malformed = encodeFastSnapshot({
            ...snapshot,
            oplog: encodeSstable(oplog, { compression: "none" }),
        });
    ...
    test("does not install shallow metadata before latest state decoding succeeds", () => {
        ...
        source.getText("text").push("b");
        source.commit();
        const snapshot = decodeFastSnapshot(source.export({ mode: "shallow-snapshot", frontiers: shallowRoot }));
        const malformed = encodeFastSnapshot({
            ...snapshot,
    ...
    test("matches sorted map and text edge contracts", () => {
        ...
        const text = doc.getText("text");
        text.insert(0, "x");
        expect(text.convertPos(0, "invalid", "unicode")).toBeUndefined();
        expect(() => text.mark({ start: 0, end: 1 }, "not-configured", true)).toThrow(/not configured/u);
        const detached = new LoroText();
        detached.insert(0, "x");
        expect(() => detached.mark({ start: 0, end: 1 }, "not-configured", true)).not.toThrow();
    });
    ...
    test("uses flattened mergeable ids and logical event paths", () => {
        ...
        expect(batches.at(-1)?.events.find((event) => event.target === counter.id)?.path).toEqual(["state", "profile", "revision"]);
    });
    ...
    test("passes live containers through toJsonWithReplacer", () => {
        ...
        body.insert(0, "hello");
        const seen = [];
        expect(doc.toJsonWithReplacer((key, value) => {
        ...
        expect(() => doc.toJsonWithReplacer((_key, value) => (value instanceof LoroText ? root : value))).toThrow(/different container/u);
    });
    ...
    test("synchronizes postcard-compatible awareness state", () => {
        const a = new AwarenessWasm("1", 30000);
        const b = new AwarenessWasm("2", 30000);
        ...
        const wrapped = new Awareness("3", 30000);
        const events = [];
    ...
    test("synchronizes typed ephemeral state and deletion tombstones", async () => {
        const a = new EphemeralStoreWasm(30000);
        const b = new EphemeralStoreWasm(30000);
        const localUpdates = [];
        const events = [];
        ...
        const typed = new EphemeralStore(30000);
        typed.set("name", "Ada");
    ...
    test("provides compatible detached classes and version vectors", () => {
        ...
        expect(VersionVector.decode(version.encode()).toJSON()).toEqual(new Map([
            ["1", 2],
            ["2", 4],
        ]));
    });
    test("loads the deep value from a current Rust snapshot", () => {
        ...
        const expected = JSON.parse(readFileSync(new URL("./fixtures/rust/snapshot.deep.json", import.meta.url), "utf8"));
        const actual = doc.toJSON();
        const actualMap = actual.map;
        const expectedMap = expected.map;
        const actualChildMap = actualMap.child_map;
        ...
        expect(actual.tree.length).toBe(expected.tree.length);
