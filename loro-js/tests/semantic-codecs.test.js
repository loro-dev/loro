...
import { ContainerType, bytesToHex, decodeChangeBlockKey, decodeContainerId, decodePostcardContainerId, decodePostcardFrontiers, decodePostcardOptionalContainerId, decodePostcardValue, decodePostcardVersionVector, encodeChangeBlockKey, encodeContainerId, encodePostcardContainerId, encodePostcardFrontiers, encodePostcardOptionalContainerId, encodePostcardValue, encodePostcardVersionVector, } from "../src/codec/index";
describe("IDs and container IDs", () => {
    test("matches the Rust ChangeStore key layout", () => {
        const id = { peer: 0x0102030405060708n, counter: -1 };
...
describe("VersionVector and Frontiers", () => {
    test("preserves VersionVector order", () => {
        ...
        expect(decodePostcardVersionVector(encodePostcardVersionVector(version))).toEqual(version);
    });
...
describe("postcard LoroValue", () => {
    ...
    test.each(values)("round trips $type", (value) => {
        ...
        expect(value.type !== "double" || Object.is(decoded.value, -0)).toBe(true);
    });
});
