...
describe("current Rust interoperability", () => {
  ...
  test("decodes every semantic table in the Rust change blocks", () => {
    ...
    for (const bytes of blocks) {
      ...
      expect(operations.reduce((sum, operation) => sum + operation.length, 0)).toBe(
        block.counterLength,
      );
      expect(
        operations.every((operation) => operation.containerIndex < containers.length),
      ).toBe(true);
      expect(
        deletes.every((entry) => entry.peerIndex < BigInt(header.peers.length)),
      ).toBe(true);
      expect(remainingValues).toHaveLength(0);
      changeCount += block.changeCount;
      nestedContainerCount += containers.filter(
        (container) => container.kind === "normal",
      ).length;
    ...
    expect([...valueTypes]).toEqual(
      expect.arrayContaining(["loro-value", "string", "raw-tree-move"]),
    );
  });
  ...
  test("decodes semantic keys and versions from a Rust snapshot", () => {
    ...
    const blockIds = history
      .filter(
        (entry) =>
          !bytesEqual(entry.key, VV_KEY) && !bytesEqual(entry.key, FRONTIERS_KEY),
      )
    ...
    expect(rootNames).toEqual(
      expect.arrayContaining(["map", "list", "text", "mlist", "tree"]),
    );
