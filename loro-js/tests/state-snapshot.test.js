...
import { ContainerType, decodeContainerStateSnapshot, decodeContainerStateWrapper, decodeStateSnapshotStore, encodeContainerStateSnapshot, encodeContainerStateWrapper, encodePositionArena, encodeStateSnapshotStore, unknownContainerType, } from "../src/codec/index";
const peer = 0x0102030405060708n;
describe("container state snapshot codec", () => {
    test("canonicalizes and round trips map state", () => {
        ...
        expect(decodeContainerStateSnapshot(ContainerType.Map, encodeContainerStateSnapshot(state))).toEqual({
            ...state,
            values: [...state.values].reverse(),
            deletedKeys: [...state.deletedKeys].reverse(),
            metadata: [...state.metadata].sort((left, right) => left.key.localeCompare(right.key)),
        });
    });
    test("round trips list, text, tree, movable-list, counter and unknown state", () => {
        const states = [
            ...
            { kind: ContainerType.Counter, bits: 0x3ff0000000000000n },
            { kind: unknownContainerType(19), payload: Uint8Array.of(1, 2, 3) },
        ];
        for (const state of states) {
            expect(decodeContainerStateSnapshot(state.kind, encodeContainerStateSnapshot(state))).toEqual(state);
        }
    });
    test("round trips empty tree position arena in its non-empty Rust encoding", () => {
        ...
        expect(decodeContainerStateSnapshot(ContainerType.Tree, encodeContainerStateSnapshot(state))).toEqual(state);
    });
});
describe("state wrapper and store codec", () => {
    ...
    test("round trips a wrapper", () => {
        expect(decodeContainerStateWrapper(encodeContainerStateWrapper(wrapper))).toEqual(wrapper);
    });
    test("preserves absent and empty stores", () => {
        for (const store of [{ kind: "absent" }, { kind: "empty" }]) {
    ...
    test("round trips an SSTable store", () => {
        ...
        expect(decodeStateSnapshotStore(encodeStateSnapshotStore(store, { compression: "none" }))).toEqual(store);
    });
});
