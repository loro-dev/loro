...
interface TestElement {
  readonly id: { readonly peer: bigint; readonly counter: number };
  readonly deletedBy: { readonly peer: bigint; readonly counter: number }[];
  deleted: boolean;
...
describe("runtime indexes", () => {
  test("maintains sequence ranks, metrics, and id lookups", () => {
    ...
    const visible = (): TestElement[] => physical.filter((element) => !element.deleted);
    ...
    for (let step = 0; step < 2_000; step += 1) {
      ...
      expect(sequence.visibleUtf16Length).toBe(
        expectedVisible.reduce((length, element) => length + element.value.length, 0),
      );
      expect(sequence.visibleUtf8Length).toBe(
        new TextEncoder().encode(expectedVisible.map(({ value }) => value).join(""))
          .length,
      );
      ...
      expect(sequence.visibleRange(start, end)).toEqual(
        expectedVisible.slice(start, end),
      );
      expect(sequence.visibleIdRuns(start, end)).toEqual(
        compressIdRuns(expectedVisible.slice(start, end)),
      );
    }
  });
  ...
  test("stops visible range predicates after the first match", () => {
    ...
    sequence.insertAtVisible(
      0,
      Array.from({ length: 96 }, (_, counter) => ({
        id: { peer: 1n, counter },
        deleted: counter % 7 === 0,
        deletedBy: [],
    ...
    let visits = 0;
    expect(
      sequence.someVisibleRange(10, 80, (element) => {
        visits += 1;
    ...
    visits = 0;
    expect(
      sequence.someVisibleRange(10, 20, () => {
        visits += 1;
        return false;
      }),
    ).toBe(false);
  ...
  test("maps id runs to visible metric ranges without crossing inserts", () => {
    ...
    expect(
      sequence.visibleMetricRangesForIdRuns(
        [{ start: { peer: 1n, counter: 0 }, length: 5 }],
        "utf16",
      ),
    ).toEqual([
      { start: 0, end: 3 },
  ...
  test("maintains ordered ranks through insertion and deletion", () => {
    const index = new OrderedIndex<{ key: number }>(
      (left, right) => left.key - right.key,
    );
    const values = Array.from({ length: 1_000 }, (_, key) => ({ key }));
    for (let offset = 0; offset < 10; offset += 1) {
      for (let key = offset; key < values.length; key += 10) index.add(values[key]!);
    }
    ...
    for (const value of values.filter(({ key }) => key % 3 === 0)) index.delete(value);
  ...
  test("indexes text styles by id runs and causal version", () => {
    const styles = new TextStyleIndex<{
      startId: { peer: bigint; counter: number };
      lamport: number;
      info: number;
      value: boolean | null;
    }>();
  ...
  test("indexes forward and reverse deletion runs in both directions", () => {
    ...
    expect(
      deletions.someDeletion({ peer: 1n, counter: 15 }, (id) => id.counter < 201),
    ).toBe(true);
    expect(
      deletions.everyDeletion({ peer: 1n, counter: 13 }, (id) => id.counter >= 100),
    ).toBe(true);
  ...
  test("indexes visibility at an earlier causal version", () => {
    ...
    const beforeConcurrentPeer = sequence.causalView(
      new Map([
        [1n, 2],
        [2n, 0],
      ]),
    ...
    expect(
      immutableView.range(0, immutableView.length).map(({ value }) => value),
    ).toEqual(["a", "c"]);
    expect(
      sequence.causalView(
        new Map([
    ...
    const withoutPeerOneTail = sequence.causalView(
      new Map([
        [1n, 1],
        [2n, 2],
      ]),
    ...
    expect(
      withoutPeerOneTail.range(0, withoutPeerOneTail.length).map(({ value }) => value),
    ).toEqual(["b"]);
    expect(
      sequence.causalView(
        new Map([
    ...
    const latest = sequence.causalView(
      new Map([
        [1n, 2],
        [2n, 2],
      ]),
    ...
    const afterMove = sequence.causalView(
      new Map([
        [1n, 2],
        [2n, 0],
      ]),
  ...
  test("packs batch inserts into bounded sequence spans", () => {
    const sequence = new SequenceIndex<TestElement>();
    const elements = Array.from({ length: 10_000 }, (_, counter) => ({
      id: { peer: 1n, counter },
      deleted: false,
      deletedBy: [],
      value: "x",
    ...
    expect(sequence.spanCount).toBeLessThanOrEqual(Math.ceil(elements.length / 32));
    expect(sequence.visibleLength).toBe(
      elements.filter((element) => !element.deleted).length,
    );
  });
  ...
  test("materializes lazy span deletions before restoring or restructuring", () => {
    ...
    sequence.setDeleted(elements[5_000]!, false);
    expect(sequence.visible()).toEqual([elements[5_000]]);
    expect(sequence.visibleIndexOf(elements[5_000]!)).toBe(0);
    expect(sequence.atPhysical(4_999)?.deleted).toBe(true);

    const inserted: TestElement = {
      id: { peer: 3n, counter: 0 },
      deleted: false,
      deletedBy: [],
      value: "inserted",
    };
    sequence.insertAtPhysical(5_000, [inserted]);
    expect(sequence.visible()).toEqual([inserted, elements[5_000]]);
  ...
  test("keeps rank and range queries correct after lazy range restoration", () => {
    ...
    sequence.setIdRunsVisible([{ start: { peer: 1n, counter: 100 }, length: 9_800 }]);

    expect(sequence.visibleLength).toBe(9_800);
    expect(sequence.atVisible(0)).toBe(elements[100]);
    expect(sequence.atVisible(5_000)).toBe(elements[5_100]);
    expect(sequence.atVisible(9_799)).toBe(elements[9_899]);
    expect(sequence.visibleIndexOf(elements[5_100]!)).toBe(5_000);
    expect(sequence.metricOffsetAtVisibleIndex(5_000, "utf16")).toBe(5_000);
    expect(sequence.visibleIndexAtMetricOffset(5_000, "utf16")).toBe(5_000);
    expect(sequence.visibleIdRuns(123, 9_123)).toEqual([
      { start: { peer: 1n, counter: 223 }, length: 9_000 },
    ]);
    expect(sequence.visibleRange(4_999, 5_002)).toEqual([
      elements[5_099],
      elements[5_100],
      elements[5_101],
    ]);
    expect(sequence.previousVisible(elements[100]!)).toBeUndefined();
    expect(sequence.nextVisible(elements[9_899]!)).toBeUndefined();
  });
  ...
  test("deletes id spans while preserving causal delete order", () => {
    ...
    expect(sequence.elementsDeletedBy(2n, 100, 150).map(({ id }) => id.counter)).toEqual(
      Array.from({ length: 50 }, (_, counter) => counter + 20),
    );
    const halfway = sequence.causalView(
      new Map([
        [1n, 96],
        [2n, 125],
      ]),
    ...
    const reversed = new SequenceIndex<TestElement>();
    ...
    const partialReverse = reversed.causalView(
      new Map([
        [1n, 8],
        [2n, 12],
      ]),
    ...
    expect(
      partialReverse.range(0, partialReverse.length).map(({ id }) => id.counter),
    ).toEqual([0, 1, 2, 3, 6, 7]);
  });
  ...
  test("coalesces adjacent single-element inserts into bounded spans", () => {
    ...
    expect(sequence.allLength).toBe(10_000);
    expect(sequence.spanCount).toBe(Math.ceil(10_000 / 32));
    for (const counter of [0, 31, 32, 4_999, 9_999]) {
  ...
  test("seeks sparse operation counters without walking numeric gaps", () => {
    const sequence = new SequenceIndex<TestElement>();
    const first: TestElement = {
      id: { peer: 1n, counter: 0 },
      deleted: false,
      deletedBy: [],
      value: "a",
    };
    const distant: TestElement = {
      id: { peer: 1n, counter: 1_000_000_000 },
      deleted: false,
      deletedBy: [],
      value: "b",
    };
    ...
    sequence.addDeletion(first, { peer: 2n, counter: 1_000_000_000 });

    const early = sequence.causalView(
      new Map([
        [1n, 1],
        [2n, 0],
      ]),
    ...
    expect(sequence.elementsDeletedBy(2n, 999_999_999, 1_000_000_001)).toEqual([first]);
  });
  ...
  test("promotes a deletion counter to a set only for one-to-many deletes", () => {
    ...
    const beforeDelete = sequence.causalView(
      new Map([
        [1n, 3],
        [2n, 0],
        [3n, 0],
    ...
    const afterSecondDelete = sequence.causalView(
      new Map([
        [1n, 3],
        [2n, 0],
        [3n, 1],
    ...
    expect(afterSecondDelete.range(0, afterSecondDelete.length)).toEqual(
      elements.slice(1),
    );
  });
  ...
  test("merges out-of-order counter runs", () => {
    const sequence = new SequenceIndex<TestElement>();
    const counters = [10, 0, 2, 1, 1_000_000_000, 11];
...
function compressIdRuns(
  elements: readonly TestElement[],
): { start: TestElement["id"]; length: number }[] {
  const runs: { start: TestElement["id"]; length: number }[] = [];
