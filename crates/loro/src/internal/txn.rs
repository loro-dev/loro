...
impl Transaction {
    ...
    pub fn new_with_origin(doc: Arc<LoroDocInner>, origin: InternalString) -> Self {
        ...
        state_lock.start_txn(origin, crate::internal::event::EventTriggerKind::Local);
    ...
    fn _commit(&mut self) -> Result<Option<CommitOptions>, LoroError> {
        ...
        state.commit_txn(
            ...
            diff.map(|arr| InternalDocDiff {
                by: crate::internal::event::EventTriggerKind::Local,
                ...
                diff: Cow::Owned(
                    arr.into_iter()
                        .map(|x| InternalContainerDiff {
                            ...
                            bring_back: false,
                            diff: (x.diff.into()),
                            diff_mode: crate::internal::diff_calc::DiffMode::Linear,
                        })
                        .collect(),
                ),
    ...
    pub(crate) fn set_default_options(&mut self, default_options: crate::internal::loro::CommitOptions) {
        if self.origin.is_empty() {
