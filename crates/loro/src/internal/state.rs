...
impl DocState {
    ...
    pub fn new_arc(
        ...
        Arc::new(lock_group.new_lock(
            ...
            crate::internal::lock::LockKind::DocState,
        ))
    }
    ...
    pub(crate) fn apply_diff(&mut self, mut diff: InternalDocDiff<'static>, diff_mode: DiffMode) {
        ...
        for mut diff in std::mem::replace(&mut diffs, Vec::with_capacity(len)) {
            ...
            match &internal_diff {
                crate::internal::event::DiffVariant::None => {
                    if is_recording {
                ...
                crate::internal::event::DiffVariant::Internal(_) => {
                ...
                crate::internal::event::DiffVariant::External(_) => unreachable!(),
            }
    ...
    pub fn get_relative_position(&mut self, pos: &Cursor, use_event_index: bool) -> Option<usize> {
        ...
        if let Some(id) = pos.id {
            ...
            if matches!(pos.side, crate::internal::cursor::Side::Left) {
                return Some(0);
            }
