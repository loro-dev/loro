...
mod dfs {
    ...
    #[allow(dead_code)]
    use super::*;
    use crate::internal::{allocation::calc_critical_version_dfs, delta::DeltaValue};
...
fn test_alloc() {
    ...
    for _ in 1..10000 {
        ...
        let end = crate::internal::allocation::get_end_list(a, start);
        let critical_version_actual: HashSet<ID> =
            crate::internal::allocation::calc_critical_version(a, start)
                .into_iter()
                .collect();
        let critical_version: HashSet<ID> =
            crate::internal::allocation::calc_critical_version_dfs(a, start, &end)
                .into_iter()
                .collect();
...
pub fn fuzz_alloc_tree(num: usize, mut actions: Vec<Interaction>) {
    ...
    let start = a.frontier();
    let end = crate::internal::allocation::get_end_list(a, start);
    let actual: HashSet<ID> = crate::internal::allocation::calc_critical_version(a, start)
        .into_iter()
        .collect();
    let expected: HashSet<ID> = crate::internal::allocation::calc_critical_version_dfs(a, start, &end)
        .into_iter()
        .collect();
...
// #[cfg(test)]
// mod find_path {
//     use crate::internal::{fx_map, span::CounterSpan};
...
//     use crate::internal::{dag::iter::IterReturn, tests::PROPTEST_FACTOR_10};
