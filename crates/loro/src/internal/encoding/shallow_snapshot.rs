...
fn calc_shallow_doc_start(oplog: &crate::internal::OpLog, frontiers: &Frontiers) -> Frontiers {
    ...
    for id in start.iter() {
        ...
        if let Some(op) = oplog.get_op_that_includes(id) {
            if let crate::internal::op::InnerContent::List(InnerListOp::StyleStart { .. }) = &op.content {
