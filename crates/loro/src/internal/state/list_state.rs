...
impl ContainerState for ListState {
    ...
    fn apply_diff_and_convert(
        ...
        for span in delta.iter() {
            match span {
                crate::internal::delta::DeltaItem::Retain { retain: len, .. } => {
                    index += len;
                    ans.push_retain(*len, Default::default());
                }
                crate::internal::delta::DeltaItem::Insert { insert: value, .. } => {
                    let mut arr = Vec::new();
                    match &value.values {
                ...
                crate::internal::delta::DeltaItem::Delete { delete: len, .. } => {
...
mod snapshot {
    ...
    use crate::internal::{encoding::value_register::ValueRegister, state::ContainerCreationContext};
...
mod test {
    ...
    use crate::internal::state::ContainerCreationContext;
    ...
    fn test() {
        ...
        fn id(name: &str) -> ContainerID {
            ContainerID::new_root(name, crate::internal::ContainerType::List)
        }
