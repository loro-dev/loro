...
impl ContainerHistoryCache {
    ...
    fn init_cache_by_visit_all_change_slow(&mut self, for_checkout: bool, for_importing: bool) {
        ...
        if let Some(state) = self.shallow_root_state.as_ref() {
            ensure_cov::notify_cov(
                "loro::internal::history_cache::init_cache_by_visit_all_change_slow::visit_gc",
            );
            ...
            for (idx, c) in store.iter_all_containers_mut() {
                ...
                match state {
                    crate::internal::state::State::MapState(m) => {
                        if for_checkout {
                    ...
                    crate::internal::state::State::MovableListState(l) => {
                    ...
                    crate::internal::state::State::TreeState(t) => {
                        if for_importing {
    ...
    pub(crate) fn find_text_chunks_in(
        ...
        ensure_cov::notify_cov("loro::internal::history_cache::find_text_chunks_in");
    ...
    pub(crate) fn find_list_chunks_in(
        ...
        ensure_cov::notify_cov("loro::internal::history_cache::find_list_chunks_in");
        let Some(state) = self.shallow_root_state.as_ref() else {
            return Vec::new();
        };
        ...
        match list_state {
            crate::internal::state::State::ListState(list) => {
            ...
            crate::internal::state::State::MovableListState(list) => {
    ...
    fn insert(&mut self, op: &RichOp) {
        ...
        match &op.op().content {
            InnerContent::List(l) => match l {
                crate::internal::container::list::list_op::InnerListOp::Move { elem_id, .. } => {
                ...
                crate::internal::container::list::list_op::InnerListOp::Set { elem_id, .. } => {
