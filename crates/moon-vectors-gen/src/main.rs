...
struct RustOracleVectorCase {
    ...
    expected_text_delta_json: Option<String>,
}
...
fn make_case_concurrent_text_insert_same_pos() -> RustOracleVectorCase {
    ...
    RustOracleVectorCase {
        ...
        updates: vec![updates_a, updates_b],
        expected_json: oracle.get_deep_value().to_json(),
        expected_text_delta_json: Some(oracle.get_text("text").get_richtext_value().to_json()),
    }
}
...
fn make_case_concurrent_richtext_bold_mark_vs_insert_end() -> RustOracleVectorCase {
    let a = doc_with_peer(1);
    ...
    a.get_text("text").mark(0..5, "bold", true).unwrap();
    commit_fixed(&a);

    b.get_text("text").insert(5, " World!").unwrap();
    commit_fixed(&b);
...
fn make_case_concurrent_richtext_link_mark_vs_insert_end() -> RustOracleVectorCase {
    let a = doc_with_peer(1);
    a.get_text("text").insert(0, "Hello").unwrap();
    commit_fixed(&a);
    ...
    a.get_text("text").mark(0..5, "link", true).unwrap();
    commit_fixed(&a);
...
fn main() {
    let cases = vec![
        ...
        make_case_concurrent_richtext_bold_mark_vs_insert_end(),
        make_case_concurrent_richtext_link_mark_vs_insert_end(),
    ];
    ...
    for c in cases {
        ...
        match &c.expected_text_delta_json {
            None => println!("    expected_text_delta_json: None,"),
            Some(s) => println!(
                "    expected_text_delta_json: Some(\"{}\"),",
                escape_moonbit_string(s)
            ),
        }
        println!("  }},");
    }
