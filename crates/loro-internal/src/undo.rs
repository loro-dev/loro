...
impl DiffBatch {
    ...
    /// If there are other diffs in the batch, the diff will be transformed by the other diffs first.
    /// Using this method can correctly push diffs generated by apply op invert functions.
    /// Like if there are continuous deletions that:
    ///
    /// "a1b2c3"
    ///
    /// - Delete 1 char at index 1 -> "ab2c3", direct inverse is insert "1" at index 1
    /// - Delete 1 char at index 2 after step 1 -> "abc3", direct inverse is insert "2" at index 2
    /// - Delete 1 char at index 3 after step 2 -> "abc", direct inverse is insert "3" at index 3
    ///
    /// If we directly compose them, the result is insert "123" at 1, which is wrong.
    ///
    /// "abc"
    ///
    /// Then to undo them, we need to
    ///
    /// - Insert "1" at index 1, "2" at index **3**, "3" at index **5**
    ///
    pub fn push_with_transform(&mut self, id: &ContainerID, mut diff: Diff) {
        if diff.is_empty() {
            return;
        }

        if let Some(this_diff) = self.cid_to_events.get_mut(id) {
            diff.transform(this_diff, true);
            this_diff.compose_ref(&diff);
        } else {
            self.cid_to_events.insert(id.clone(), diff);
            self.order.push(id.clone());
        }
    }
