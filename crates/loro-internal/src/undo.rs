...
impl DiffBatch {
    ...
    pub fn new_from_container_diff(diff: &[&ContainerDiff]) -> Self {
        let mut map: FxHashMap<ContainerID, Diff> = Default::default();
        let mut order: Vec<ContainerID> = Vec::with_capacity(diff.len());
        for item in diff.into_iter() {
            let old = map.insert(item.id.clone(), item.diff.clone());
            assert!(old.is_none());
            order.push(item.id.clone());
        }
    ...
    pub fn prepend(&mut self, id: &ContainerID, diff: Diff) {
        ...
        batch.cid_to_events.insert(id.clone(), diff);
...
impl Stack {
    ...
    fn push_with_merge(
        ...
        if can_merge {
            if last.span.end == span.start {
                ...
                last.undo_diff.compose(&undo_diff);
                return;
            }
        }
    ...
    fn compose_remote_event(&mut self, diff: &[&ContainerDiff]) {
        ...
        let diff = DiffBatch::new_from_container_diff(diff);
        for item in self.stack.iter_mut() {
            item.undo_diff.transform(&diff, true);
        }
    }
...
impl UndoManager {
    pub fn new(doc: &LoroDoc) -> Self {
        ...
        let undo_diff_sub = doc.subscribe_undo_diffs(Box::new(move |diff_batch| {
            ...
            // trace!("Received undo diff: {:#?}", diff_batch);

            if inner
                .exclude_origin_prefixes
                .iter()
                .any(|x| diff_batch.origin.starts_with(&**x))
            {
                return true;
            }

            // Store the received diff batch in pending_undo_diff
            inner.pending_undo_diff.compose(&diff_batch.diff);
            true
        }));
    ...
    fn perform(
        ...
        while let Some(mut span) = top {
            let mut next_push_selection;
            let undo_diff = span.undo_diff.clone();
            ...
            next_push_selection = self.handle_on_pop_callback(&mut span, kind, doc);
        ...
        // {
        //     let mut inner = self.inner.lock().unwrap();
        //     trace!("Undo stack: {:#?}", inner.undo_stack);
        //     trace!("Redo stack: {:#?}", inner.redo_stack);
        // }
        Ok(executed)
    }
