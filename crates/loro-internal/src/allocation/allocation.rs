...
fn inc_dep_or_ind(dep_or_ind: &mut HashMap<ID, usize>, id: &ID) {
    let x = dep_or_ind.get_mut(id).unwrap();
    *x += 1;
}
...
impl AllocationTree {
    ...
    fn run<T: DagNode, D: Dag<Node = T>>(
        ...
        self.calc::<T, D>();
        self.resolve(&end_id_set, &start_id_list)
    }
    ...
    fn resolve(&self, end_id_set: &HashSet<ID>, start_id_list: &[ID]) -> Vec<ID> {
        let mut result: HashSet<ID> = HashSet::new();
        for &u in start_id_list {
            let mut ux = u;
            result.insert(ux);
            while !end_id_set.contains(&ux) {
