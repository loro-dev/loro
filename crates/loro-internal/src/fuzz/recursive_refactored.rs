...
//         match type_ {
//             ContainerType::Text => self.text_containers.push(idx.try_into().unwrap()),
//             ContainerType::Map => self.map_containers.push(idx.try_into().unwrap()),
//             ContainerType::List => self.list_containers.push(idx.try_into().unwrap()),
//         }
//     }
// }
...
//                 let txn = self[*site as usize].loro.txn().unwrap();
//                 if let Some(list) = self[*site as usize]
//                     .list_containers
//                     .get(*container_idx as usize)
//                 {
//                     *key %= (list.len(&txn) as u8).max(1);
//                     if *value == FuzzValue::Null && list.is_empty(&txn) {
...
//                     .get(*container_idx as usize)
//                 {
//                     *pos %= (text.len(&txn) as u8).max(1);
//                     if *is_del {
//                         *value &= 0x1f;
//                         *value = (*value).min(text.len(&txn) as u16 - (*pos) as u16);
//                     }
//                 } else {
//                     *is_del = false;
//                     *pos = 0;
