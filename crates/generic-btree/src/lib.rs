...
type LeafDirtyMap<Diff> = Vec<(ArenaIndex, Diff)>;
...
impl<B: BTreeTrait> BTree<B> {
    ...
    fn create_subtrees_from_elem(&mut self, data: Vec<B::Elem>) -> (ArenaIndex, usize) {
        ...
        while let Some(first_elem) = elems.next() {
            ...
            let parent = self.in_nodes.get_mut(parent_index).unwrap();
            let mut next_elem = Some(first_elem);
            for _ in 0..MAX_CHILDREN_NUM {
                let Some(elem) = next_elem.take().or_else(|| elems.next()) else {
                    break;
                };
                let leaf = {
                ...
                parent.children.push(leaf).unwrap();
            }
        }
        ...
        while nodes.len() > 1 {
            ...
            while let Some(first_child_idx) = node_iter.next() {
                let parent_index = self.in_nodes.insert(Node {
                    parent: None,
                    parent_slot: 0,
                    children: Default::default(),
                ...
                let mut next_child_idx = Some(first_child_idx);
                for i in 0..MAX_CHILDREN_NUM {
                    let Some(child_idx) = next_child_idx.take().or_else(|| node_iter.next()) else {
                        break;
                    };
                    ...
                    parent.children.push(child_info).unwrap();
                }
            }
            nodes = new_nodes;
            height += 1;
        ...
        F: FnMut(&mut B::Elem) -> Option<B::CacheDiff>,
    {
        ...
        let mut dirty_map: LeafDirtyMap<B::CacheDiff> = Vec::new();
    ...
    pub fn update_leaf(
        ...
        let (need_update_cache, new_insert_1, new_insert_2) = {
            let node = self.leaf_nodes.get_mut(node_idx.0).unwrap();
            f(&mut node.elem)
        };

        self.finish_update_leaf(node_idx, need_update_cache, new_insert_1, new_insert_2)
    }

    fn finish_update_leaf(
        &mut self,
        node_idx: LeafIndex,
        need_update_cache: bool,
        mut new_insert_1: Option<B::Elem>,
        mut new_insert_2: Option<B::Elem>,
    ) -> (bool, SplittedLeaves) {
        let mut splitted = SplittedLeaves::default();
        let node = self.leaf_nodes.get_mut(node_idx.0).unwrap();
    ...
    pub fn update_leaves_with_arg_in_ranges<A: Debug>(
        ...
        let mut dirty_map: LeafDirtyMap<B::CacheDiff> = Vec::new();
        let mut new_elems_at_cursor = Vec::new();
        let mut args = args.into_iter().peekable();
        while let Some(first) = args.next() {
            let leaf = first.0;
            let mut group = vec![first];
            while matches!(args.peek(), Some(next) if next.0 == leaf) {
                group.push(args.next().unwrap());
            }
            ...
            let mut split_at = Vec::with_capacity(group.len() * 2);
            for (_, range, _) in group.iter() {
                split_at.push(range.start);
                split_at.push(range.end);
            }

            split_at.sort_unstable();
            split_at.dedup();
            split_at.retain(|&x| x != 0 && x != len);
            ...
            add_leaf_dirty_map(leaf.into(), &mut dirty_map, diff);
        }
    ...
    fn update_dirty_cache_map(&mut self, mut diff_map: LeafDirtyMap<B::CacheDiff>) {
        ...
        let mut visit_set = Vec::new();
        for (idx, _) in &diff_map {
            push_unique_arena(&mut visit_set, *idx);
        }
            ...
            for child_idx in take(&mut visit_set) {
                let (parent_idx, cache_diff) = match child_idx {
                    ArenaIndex::Leaf(leaf_idx) => {
                        ...
                        let cache_diff = remove_dirty_map(&mut diff_map, child_idx).unwrap();
                    ...
                    ArenaIndex::Internal(_) => {
                        ...
                        let cache_diff = child.calc_cache(
                            ...
                            remove_dirty_map(&mut diff_map, child_idx),
                        );
                ...
                push_unique_arena(&mut visit_set, parent_idx);
                if let Some((_, existing)) = diff_map.iter_mut().find(|(idx, _)| *idx == parent_idx)
                {
                    B::merge_cache_diff(existing, &cache_diff);
                } else {
                    diff_map.push((parent_idx, cache_diff));
                }
            }
        }
            ...
            Some((a_idx, b_idx)) => {
                ...
                let ans = if a.len() + b.len() >= MAX_CHILDREN_NUM {
                    ...
                    let is_parent_lack = if node_idx == a_idx.arena {
                        ...
                        for (i, child) in b.children.iter().enumerate() {
                            re_parent.push((child.arena, a_idx.arena, a.children.len() + i));
                        }
                        ...
                        for (i, child) in b.children.iter().enumerate() {
                            re_parent.push((child.arena, b_idx.arena, i + a.children.len()));
                        }
    ...
    pub fn diagnose_balance(&self) {
        ...
        for (_, node) in self.in_nodes.iter() {
            increment_counter(&mut size_counter, node.children.len());
        }
        dbg!(size_counter);

        let mut size_counter = Vec::new();
        for (_, node) in self.leaf_nodes.iter() {
            increment_counter(&mut size_counter, node.elem.rle_len());
        }
        dbg!(size_counter);
    }
...
fn add_leaf_dirty_map<T>(leaf: ArenaIndex, dirty_map: &mut LeafDirtyMap<T>, leaf_diff: T) {
    if let Some((_, existing)) = dirty_map.iter_mut().find(|(idx, _)| *idx == leaf) {
        *existing = leaf_diff;
    } else {
        dirty_map.push((leaf, leaf_diff));
    }
}

fn remove_dirty_map<T>(dirty_map: &mut LeafDirtyMap<T>, leaf: ArenaIndex) -> Option<T> {
    let index = dirty_map.iter().position(|(idx, _)| *idx == leaf)?;
    Some(dirty_map.swap_remove(index).1)
}

fn push_unique_arena(vec: &mut Vec<ArenaIndex>, index: ArenaIndex) {
    if !vec.contains(&index) {
        vec.push(index);
    }
}

fn increment_counter(counter: &mut Vec<(usize, usize)>, key: usize) {
    if let Some((_, count)) = counter.iter_mut().find(|(current, _)| *current == key) {
        *count += 1;
    } else {
        counter.push((key, 1));
    }
