...
// The nodejs entry is CJS; bundling it into ESM output used to break on the
// glue's `__dirname`-bound wasm load (see patchNodejsWasmByteSource in
// build.ts). Generate a real-ESM wrapper (index.mjs) that preloads the wasm
// bytes via import.meta.url (esm-preload.mjs) and re-exports the CJS module's
// full surface, so `import` — plain node ESM and ESM bundles alike — gets the
// same module as `require("loro-crdt")`. Wired into package.json "exports"
// under the node/import conditions.
async function generateNodejsEsmWrapper() {
  await Deno.copyFile(
    "./scripts/nodejs_esm_preload.mjs",
    "./nodejs/esm-preload.mjs",
  );

  // Enumerate the CJS namespace at build time by actually requiring the built
  // entry — exact parity with what `require("loro-crdt")` exposes, including
  // the names re-exported dynamically from the glue.
  const command = new Deno.Command("node", {
    args: [
      "-e",
      "const m = require('./nodejs/index.js'); process.stdout.write(JSON.stringify(Object.keys(m)))",
    ],
  });
  const { code, stdout, stderr } = await command.output();
  if (code !== 0) {
    throw new Error(
      `Failed to enumerate nodejs entry exports:\n${new TextDecoder().decode(stderr)}`,
    );
  }
  const keys: string[] = JSON.parse(new TextDecoder().decode(stdout));
  const names = keys
    .filter((k) => k !== "default" && /^[A-Za-z_$][\w$]*$/.test(k))
    .sort();
  if (!names.includes("LoroDoc") || !names.includes("isValidContainerId")) {
    throw new Error(
      `nodejs entry export enumeration looks wrong (got ${names.length} names, missing LoroDoc/isValidContainerId)`,
    );
  }

  const lines = names.map((name) => `  ${name},`).join("\n");
  const content = `// Generated by post-rollup.ts — real-ESM wrapper for the nodejs entry.
// esm-preload.mjs MUST stay the first import: it resolves loro_wasm_bg.wasm
// via import.meta.url and hands the bytes to the CJS glue before it runs, so
// this entry survives being bundled into ESM output (no __dirname there).
import "./esm-preload.mjs";
...
`;
  await Deno.writeTextFile("./nodejs/index.mjs", content);
  console.log(
    `✓ Generated nodejs/index.mjs (${names.length} named exports) + nodejs/esm-preload.mjs`,
  );
}
...
async function main() {
  ...
  await generateNodejsEsmWrapper();
  await rollupBase64();
