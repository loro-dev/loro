...
impl Loro {
    ...
    /// Export the snapshot of current version, it's include all content of
    /// operations and states
    #[wasm_bindgen(js_name = "exportSnapshotV0")]
    pub fn export_snapshot_v0(&self) -> JsResult<Vec<u8>> {
    ...
    /// const updates2 = doc.exportFrom(version);
    /// ```
    #[wasm_bindgen(skip_typescript, js_name = "exportFromV0")]
    pub fn export_from_v0(&self, version: &JsValue) -> JsResult<Vec<u8>> {
        // `version` may be null or undefined
        let vv = js_value_to_version(version)?;
